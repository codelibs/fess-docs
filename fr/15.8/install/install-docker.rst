...
Depuis |Fess| 15.8, la fonctionnalité de mode de recherche IA (RAG Chat) a été séparée sous forme de plugins ``fess-llm-*``.
...
.. list-table::
   ...
   * - ``FESS_PLUGINS``
     - Plugins installés automatiquement au démarrage (format ``name:version`` séparés par des espaces. Exemple : ``fess-ds-wikipedia:15.8.0``)
