...
.. list-table::
   ...
   * - ``last_crawl_time``
     - Non
     - Date de référence à partir de laquelle une exploration incrémentale sélectionne les données. Réécrite automatiquement à la fin du crawl (voir « Exploration incrémentale »)
   * - ``last_crawl_time_format``
     - Non
     - Format de ``last_crawl_time``. Par défaut : ``yyyy-MM-dd HH:mm:ss``
...
Écrivez ``${last_crawl_time}`` dans ``sql`` : l'expression est remplacée par l'heure de début
du crawl précédent :
...
Lors de la première exécution, elle est remplacée par ``1970-01-01 00:00:00`` : tous les
enregistrements sont donc sélectionnés. Une fois l'intégralité du jeu de résultats lue, l'heure
de début du crawl courant est réécrite dans la configuration du DataStore sous le nom
``last_crawl_time`` et utilisée lors de l'exécution suivante.

Le format se configure avec ``last_crawl_time_format`` (par défaut ``yyyy-MM-dd HH:mm:ss``).
Il doit produire une valeur que la base de données accepte comme littéral d'horodatage.

La valeur correspond à l'heure de **début** du crawl : une ligne mise à jour pendant le crawl
est donc récupérée lors de l'exécution suivante. Rien n'est réécrit si le crawl s'arrête en
cours de route.

.. warning::
   Une exploration incrémentale ne peut pas détecter les lignes supprimées.

   L'activer désactive également le nettoyage qui supprime les documents laissés par les crawls
   précédents (``delete_old_docs``). Sans cela, chaque document inchangé serait supprimé à
   chaque exécution, puisqu'une requête incrémentale ne le renvoie pas.

   Par conséquent, un document dont la ligne a été supprimée de la base de données reste dans
   l'index jusqu'à son expiration. Exécutez périodiquement un crawl complet, c'est-à-dire une
   configuration sans ``${last_crawl_time}``.

   Un ``delete_old_docs`` défini explicitement dans la configuration du DataStore est
   prioritaire.

Écrire la condition directement dans ``sql`` fonctionne toujours comme auparavant :
