...
Dans |Fess| 15.9, la fonctionnalité LLM a été séparée en plugins ``fess-llm-*``.
...
Dans |Fess| 15.9, les paramètres sont séparés en deux familles : la famille FessConfig
...
.. note::
   ...
   Dans 15.9 et supérieur il est renommé en ``rag.llm.name`` ; les valeurs écrites
...
Dans |Fess| 15.9, les paramètres de génération (nombre maximum de tokens, température, etc.) se configurent par fournisseur
...
Dans |Fess| 15.9, les prompts système sont définis dans le DI XML (``fess_llm++.xml``) de chaque plugin ``fess-llm-*``
...
.. note::
   ...
   Dans 15.9, utilisez l'API JSON sous ``/api/v2/``.
