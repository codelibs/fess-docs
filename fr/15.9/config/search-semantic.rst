...
Dans |Fess| 15.9, la **fonctionnalité de chunking de contenu** — qui découpe le corps des
...
- **OpenSearch avec le plugin k-NN** : dans |Fess| 15.9, le mapping de l'index de recherche
  ...
  .. list-table::
     ...
     * - OpenSearch intégré (``bin/fess``, ou les paquets ZIP avec ``SEARCH_ENGINE_HTTP_URL``
...
Tout index ``fess.search`` nouvellement créé sous |Fess| 15.9 dispose toujours des réglages
...
.. note::
   ...
   15.9 ou ultérieur (réindexation comprise) applique le mapping contenant
...
15.7 et versions antérieures, a été intégré au cœur en 15.9 et est désormais **inutile
