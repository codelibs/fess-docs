======================================
OpenAI-Konfiguration (AI-Suche / RAG)
======================================

Übersicht
=========

Diese Seite erläutert, wie Sie das Plugin ``fess-llm-openai`` konfigurieren, damit |Fess| OpenAI für seinen **AI-Suchmodus (RAG: Retrieval-Augmented Generation)** nutzen kann — die Beantwortung natürlichsprachlicher Fragen aus Ihrem Unternehmens-Suchindex mit zitierten Quellen. |Fess| ruft die OpenAI-API auf, um RAG über Ihre gecrawlten Dokumente mit GPT-Modellen auszuführen.
...
1. Laden Sie `fess-llm-openai-15.8.0.jar` herunter
...
::
    ...
    # Reasoning-Effort für die Antwortgenerierung mit o3-mini auf high setzen
    rag.llm.openai.model=o3-mini
    rag.llm.openai.answer.reasoning.effort=high

    # top_p und Strafen für die Antwortgenerierung mit gpt-5 konfigurieren
...
- ``FESS_PLUGINS=fess-llm-openai:15.8.0`` lässt das ``run.sh`` des Containers das Plugin automatisch herunterladen und in ``app/WEB-INF/plugin/`` installieren
