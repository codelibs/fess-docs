...
.. list-table::
   ...
   * - ``last_crawl_time``
     - Nein
     - Referenzzeitpunkt, ab dem ein inkrementelles Crawling auswählt. Wird nach Abschluss eines Crawls automatisch zurückgeschrieben (siehe "Inkrementelles Crawling")
   * - ``last_crawl_time_format``
     - Nein
     - Format von ``last_crawl_time``. Standard: ``yyyy-MM-dd HH:mm:ss``
...
Schreiben Sie ``${last_crawl_time}`` in ``sql``, und der Ausdruck wird durch den
Startzeitpunkt des vorherigen Crawls ersetzt:

::

    sql=SELECT id, title, content, url, updated_at FROM articles WHERE updated_at > '${last_crawl_time}'

Beim ersten Lauf wird er durch ``1970-01-01 00:00:00`` ersetzt, sodass alle Datensätze
ausgewählt werden. Sobald das gesamte Resultset gelesen wurde, wird der Startzeitpunkt
dieses Crawls als ``last_crawl_time`` in die Datenspeicher-Konfiguration zurückgeschrieben
und beim nächsten Lauf verwendet.

Das Format lässt sich mit ``last_crawl_time_format`` einstellen (Standard
``yyyy-MM-dd HH:mm:ss``). Es muss etwas erzeugen, das die Datenbank als Zeitstempel-Literal
akzeptiert.

Der Wert ist der **Startzeitpunkt** des Crawls; eine Zeile, die während des laufenden Crawls
aktualisiert wird, wird daher erst beim nächsten Lauf erfasst. Wird das Crawling vorzeitig
abgebrochen, wird nichts zurückgeschrieben.

.. warning::
   Ein inkrementelles Crawling kann gelöschte Zeilen nicht erkennen.

   Wird es aktiviert, wird außerdem der Bereinigungslauf deaktiviert, der von früheren Crawls
   übrig gebliebene Dokumente entfernt (``delete_old_docs``). Andernfalls würde bei jedem Lauf
   jedes unveränderte Dokument gelöscht, da eine inkrementelle Query diese nicht zurückliefert.

   Ein Dokument, dessen Zeile aus der Datenbank gelöscht wurde, bleibt daher bis zum Ablauf im
   Index. Führen Sie regelmäßig ein vollständiges Crawling durch - eine Konfiguration ohne
   ``${last_crawl_time}``.

   Ein in der Datenspeicher-Konfiguration explizit gesetztes ``delete_old_docs`` hat Vorrang.

Die Bedingung wie bisher direkt in ``sql`` zu schreiben, funktioniert weiterhin:
