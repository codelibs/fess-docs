...
.. list-table::
   ...
   * - Groovy
     - ``groovy``
     - Wird als ``fess-script-groovy``-Plugin bereitgestellt, das **nicht Teil der
       Distribution ist**. Installieren Sie es über die Administrationsoberfläche oder
       mit ``bin/fess-setup install plugin fess-script-groovy``.
