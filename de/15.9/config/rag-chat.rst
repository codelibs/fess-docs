...
In |Fess| 15.9 wurde die LLM-Funktion als ``fess-llm-*``-Plugin ausgelagert.
...
In |Fess| 15.9 sind die Einstellungen in zwei Familien aufgeteilt: die FessConfig-Familie
...
.. note::
   ...
   In 15.9 und später wurde er in ``rag.llm.name`` umbenannt; Werte unter
...
In |Fess| 15.9 werden Generierungsparameter (maximale Token-Anzahl, Temperature usw.) pro Anbieter
...
In |Fess| 15.9 werden Systemprompts nicht in Property-Dateien, sondern im DI-XML (``fess_llm++.xml``) der jeweiligen ``fess-llm-*``-Plugins definiert.
...
.. note::
   ...
   Verwenden Sie in 15.9 die JSON-basierte API unter ``/api/v2/``.
