...
이 설정은 크롤러의 HTTP 통신에만 적용됩니다. SSO 인증이나 외부 API 연계 등의 HTTP 통신에는 적용되지 않습니다.
...
.. note::
   Fess 애플리케이션 전체(SSO, LLM 연계 등 포함)에서 프록시를 사용하는 경우 환경 변수 ``FESS_PROXY_HOST``, ``FESS_PROXY_PORT`` 를 설정하십시오. 자세한 내용은 아래의 「환경 변수를 통한 프록시 설정」을 참조하십시오.
...
.. note::
   ``fess_config.properties`` 의 ``http.proxy.*`` 설정은 크롤러의 기본 프록시로 사용됩니다.
   SSO 인증이나 LLM 연계를 포함한 Fess 애플리케이션 전체에서 프록시를 사용하는 경우 위의 환경 변수를 설정하십시오.
