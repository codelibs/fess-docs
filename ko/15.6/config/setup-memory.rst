...
``FESS_HEAP_SIZE`` 는 ``-Xms`` 와 ``-Xmx`` 를 동일한 값으로 설정합니다. 초기 힙 크기와 최대 힙 크기를 개별적으로 제어하려면 ``FESS_MIN_MEM`` 과 ``FESS_MAX_MEM`` 환경 변수를 사용하세요.
...
.. note::
   ``FESS_HEAP_SIZE`` 가 설정된 경우 ``FESS_MIN_MEM`` 과 ``FESS_MAX_MEM`` 은 무시됩니다.
...
.. list-table::
   ...
   * - ``-XX:MaxGCPauseMillis=60000``
     - 최대 GC 일시 정지 목표 시간(60초). 크롤러 워크로드에 대한 완화된 설정(일반 애플리케이션에서는 약 200ms가 일반적)
...
OpenSearch의 힙 메모리는 ``config/jvm.options`` 파일에서 지정하는 것을 권장합니다.
