...
- **표준 오브젝트**: 미리 정의된 표준 오브젝트(Account, Contact, Lead, Opportunity, Case, Solution 등). 대상 표준 오브젝트는 고정되어 있으며, 크롤링 실행 시 모두 가져옵니다.
- **사용자 정의 오브젝트**: ``custom`` 파라미터로 지정한 사용자 정의 오브젝트 (API 이름이 ``__c`` 로 끝나는 오브젝트).

.. note::

   표준 오브젝트는 항상 전체 크롤링됩니다(크롤링할 표준 오브젝트를 개별적으로 선택할 수 없습니다).
   불필요한 오브젝트를 제외하려면 ``include_pattern`` / ``exclude_pattern`` 을 사용한 URL 필터링을 이용하세요.

표준 오브젝트 목록
------------------

크롤링 대상 표준 오브젝트는 다음과 같습니다.
"오브젝트명"은 필드 매핑(``<오브젝트명>.title`` 등)에서 사용하는 식별자이며,
``object.type`` 은 스크립트에서 참조할 수 있는 오브젝트 종류 값입니다.
...
.. note::
   ...
   표준 오브젝트의 필드 매핑에서는 오브젝트명에 대문자 스네이크 케이스
   (`표준 오브젝트 목록`_ 의 "오브젝트명" 열 값)를 사용합니다
   (예: ``ACCOUNT.title=Name``, ``DAND_B_COMPANY.title=Name``).
   사용자 정의 오브젝트의 경우 API 이름을 그대로 사용합니다(예: ``Product__c.title=Name``).
