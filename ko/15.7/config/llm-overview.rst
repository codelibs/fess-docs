...
기본값으로 키워드(BM25) 검색이 사용됩니다. 이 표준 파이프라인을 재사용하기 때문에 Semantic Search 플러그인을
도입·설정하면 해당 semantic(벡터) 서처가 AI 검색 모드의 검색 단계를 포함한 모든 검색에서 Rank Fusion에
참여합니다. AI 검색 모드 전용 설정은 필요하지 않습니다. 자세한 내용은 :doc:`rank-fusion` 을 참조하세요.
