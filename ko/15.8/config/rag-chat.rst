...
|Fess| 15.8에서는 LLM 기능이 ``fess-llm-*`` 플러그인으로 분리되었습니다.
...
AI 검색 모드는 전용 벡터 인덱스가 아닌 |Fess|\ 의 표준 검색 파이프라인(Rank Fusion)을 통해
소스 문서를 취득합니다. 역할·레이블에 의한 |Fess| 통상의 접근 제어도 그대로 적용됩니다.
기본값으로 키워드(BM25) 검색이 사용되며, LLM 자체가 문서의 검색·순위 결정·임베딩
(embedding)을 수행하지 않습니다.

요청 종류에 따라 실행되는 파이프라인이 약간 다릅니다.

- ``POST /api/v2/chat/stream`` (웹 UI에서 사용)은 **의도 검출 → 검색 → LLM에 의한 관련도 평가 → 내용 취득 → 응답 생성** 의 전체 흐름을 실행합니다(스트리밍).
- ``POST /api/v2/chat`` (비스트리밍)은 **의도 검출 → 검색 → 응답 생성** 의 축소된 흐름을 실행합니다(관련도 평가 및 독립된 내용 취득 단계는 없습니다).

스트리밍 흐름에서는 추가 LLM 호출을 통해 **검색 결과를 평가한 뒤**, 관련성이 있다고 판단된 문서만 응답 생성 전 단계까지 남깁니다.
...
|Fess| 15.8에서는 설정이 두 가지 계열로 분리되어 있습니다: FessConfig 계열
...
.. note::
   ...
   15.8 이후에는 ``rag.llm.name`` 으로 변경되었으며, ``rag.llm.type`` 에 작성된 값은
   읽혀지지 않습니다.
...
.. list-table::
   ...
   * - ``rag.chat.content.fulltext.max.length``
     - 이 값을 초과하는 ``content_length`` 의 문서는 회답 컨텍스트에서 본문 전체가 아닌 하이라이트 발췌를 사용하는 임계값
     - ``3000``
   * - ``rag.chat.answer.highlight.fragment.size``
     - 대형 문서에서 회답 컨텍스트용으로 발췌할 때의 하이라이트 프래그먼트 크기
     - ``1000``
   * - ``rag.chat.answer.highlight.number.of.fragments``
     - 대형 문서에서 회답 컨텍스트용으로 발췌할 때의 하이라이트 프래그먼트 수
     - ``5``
   ...
   * - ``rag.chat.history.titles.max.count``
     - ``smart_summary`` 모드에서 턴별로 유지할 참조 문서 타이틀의 최대 수
     - ``5``

생성 파라미터
================

|Fess| 15.8에서는 생성 파라미터(최대 토큰 수, temperature 등)를 프로바이더별,
...
|Fess| 15.8에서는 시스템 프롬프트가 프로퍼티 파일이 아닌 각 ``fess-llm-*``
...
- 퍼밋 취득 대기가 ``concurrency.wait.timeout`` 을 초과한 경우 타임아웃 오류가 발생합니다
...
.. list-table::
   ...
   * - 모드
     - 설명
   * - ``smart_summary``
     - (기본값) 어시스턴트 응답 본문은 이력에서 생략하고, 턴별로 과거 검색 쿼리와 참조 문서 타이틀(최대 ``rag.chat.history.titles.max.count`` 건)만 유지
   * - ``full``
     - 응답 전체를 그대로 보존
   * - ``source_titles``
     - 소스 타이틀만 보존
...
.. note::
   ...
   ``smart_summary`` 모드에서는 응답 본문을 검색 쿼리와 참조 타이틀로 대체함으로써 문맥을 효율적으로 보존하면서 토큰 사용량을 줄입니다.
   사용자와 어시스턴트의 메시지 쌍이 턴 단위로 그룹화되어 문자 수 예산 내에서 최적으로 패킹됩니다.
...
AI 검색 모드 기능은 REST API(v2 API)를 통해 이용할 수 있습니다.
베이스 URL은 ``http://<서버 이름>/api/v2/`` 입니다.

Chat API는 다음 3개의 엔드포인트를 제공합니다.
...
.. list-table::
   ...
   * - 엔드포인트
     - 설명
   * - ``POST /api/v2/chat``
     - 일괄(비스트리밍) RAG 채팅 컴플리션
   * - ``POST /api/v2/chat/stream``
     - 스트리밍 RAG 채팅 컴플리션(Server-Sent Events)
   * - ``DELETE /api/v2/chat/sessions/{session_id}``
     - 채팅 세션의 대화 기록 삭제

요청은 ``Content-Type: application/json`` 의 JSON 본문으로 송신합니다.
상태를 변경하는 요청( ``POST`` / ``DELETE`` )에는 CSRF 토큰( ``X-Fess-CSRF-Token`` 헤더)이 필요합니다.
응답은 공통 엔벨로프 ``response`` 에 저장됩니다.

.. note::

   |Fess| 15.5 이전에 제공되었던 ``/api/v1/chat`` 계열의 폼 파라미터 형식 엔드포인트는 폐지되었습니다.
   15.8에서는 ``/api/v2/`` 하위의 JSON 기반 API를 사용하세요.

비스트리밍 API
-------------------

엔드포인트: ``POST /api/v2/chat``
...
.. list-table::
   ...
   * - ``session_id``
     - 아니오
     - 세션 ID(대화를 계속하는 경우). 생략 시 서버가 생성하여 응답으로 반환합니다
   * - ``fields``
     - 아니오
     - 취득 단계용 임의 필터 필드(object)
   * - ``fields.label``
     - 아니오
     - 레이블에 의한 검색 필터
   * - ``extra_queries``
...
엔드포인트: ``POST /api/v2/chat/stream``

요청 본문은 ``POST /api/v2/chat`` 와 동일(JSON)입니다.
...
.. list-table::
   ...
   * - 이벤트
     - 설명(페이로드)
   * - ``phase``
     - 처리 단계의 시작/완료( ``intent``, ``search``, ``evaluate``, ``fetch``, ``answer`` ). ``{ phase, status, message?, keywords?, hit_count?, ... }``
   * - ``chunk``
     - 생성된 텍스트 단편( ``{ content }`` )
   * - ``retry``
     - LLM 요청이 재시도될 때 통지( ``{ phase, operation, attempt, max_attempts, sleep_ms, cause? }`` )
   * - ``waiting``
     - 동시 실행 퍼밋 취득 대기 등 장시간 단계의 진행 상황( ``{ phase, reason, elapsed_ms, timeout_ms }`` )
   * - ``fallback``
     - 검색 결과 0건 등으로 쿼리가 재생성되었을 때 통지( ``{ phase, reason, original_query?, new_query? }`` , 이유는 ``no_results`` 또는 ``no_relevant_results`` )
   * - ``warning``
     - 복구 가능한 경고 발생 시 통지( ``{ phase, code, detail? }`` . 추론 모델의 토큰 고갈 등)
   * - ``sources``
     - 참조 문서 정보( ``{ sources: [...] }`` )
   * - ``done``
     - 처리 완료( ``{ session_id, html_content? }`` ). ``html_content`` 에는 Markdown 렌더링된 HTML 문자열이 포함됨
   * - ``error``
     - 종단 스트림 중도 실패( ``{ phase?, message, error_code }`` ). 타임아웃, 컨텍스트 길이 초과, 모델 미발견, 잘못된 응답, 연결 오류 등

세션 삭제
------------------

엔드포인트: ``DELETE /api/v2/chat/sessions/{session_id}``

지정한 세션의 대화 기록을 삭제합니다. 성공 시 ``cleared: true`` 가 반환됩니다.

자세한 API 문서(인증, CSRF, 속도 제한, HTTP 상태 코드)는 :doc:`../api/api-chat` 를 참조하세요.
...
2. 해당 ``fess-llm-*`` 플러그인이 설치되어 있는가
   ...
   - Docker: ``FESS_PLUGINS=fess-llm-gemini:15.8.0`` (또는 ``fess-llm-openai`` / ``fess-llm-ollama`` )가 지정되어 있는가
   - 패키지 설치: JAR 파일이 ``app/WEB-INF/plugin/`` 에 배치되어 있는가
   - 시작 로그에 ``Installing fess-llm-XXX-15.8.0.jar`` 이 출력되고 있는가
...
6. LLM 프로바이더로의 네트워크 연결이 가능한가
   ...
   - 프록시 경유가 필요한 경우, ``fess_config.properties`` 의 ``http.proxy.host`` / ``http.proxy.port`` (필요에 따라 ``http.proxy.username`` / ``http.proxy.password`` )를 설정하세요. Docker 환경에서는 ``FESS_JAVA_OPTS`` 에 ``-Dfess.config.http.proxy.host=... -Dfess.config.http.proxy.port=...`` 를 추가합니다( |Fess| 15.8 이후, LLM 클라이언트는 |Fess| 공통의 프록시 설정을 참조합니다)
