...
.. warning::
   ``FESS_PORT`` 환경 변수는 Linux 환경에서만 사용할 수 있습니다. Windows 환경에서는 ``bin\fess.in.bat`` 의 ``-Dfess.port`` 를 직접 편집하세요.
...
Linux 환경에서는 환경 변수 ``FESS_CONTEXT_PATH`` 를 사용할 수도 있습니다.
...
.. warning::
   ``FESS_CONTEXT_PATH`` 환경 변수는 Linux 환경에서만 사용할 수 있습니다. Windows 환경에서는 ``FESS_JAVA_OPTS`` 에서 ``-Dfess.context.path`` 를 직접 설정하세요.
...
크롤러의 프록시 설정에서는 특정 호스트의 제외(nonProxyHosts)는 지원되지 않습니다.
특정 호스트를 프록시에서 제외하는 경우 환경 변수 ``FESS_NON_PROXY_HOSTS`` 를 사용하십시오.
이 설정은 Java 시스템 속성 ``http.nonProxyHosts`` 로 변환되어 Fess 전체의 HTTP 통신에 적용됩니다.
...
크롤 설정에서 개별적으로 ``client.proxyHost`` 를 지정하지 않은 경우의 기본 프록시를 ``fess_config.properties`` 에서 설정할 수 있습니다.
이 설정은 크롤러의 기본 프록시뿐만 아니라 LLM 연동의 HTTP 통신과 플러그인 다운로드에도 적용됩니다. 다만 SSO 인증 등 일부 Java 라이브러리의 통신에는 적용되지 않습니다.
SSO 인증을 포함한 Fess 애플리케이션 전체에 프록시를 적용하려면 아래에서 설명하는 환경 변수(``FESS_PROXY_HOST`` 등)를 사용하십시오.
...
.. note::
   ``http.proxy.*`` 설정은 SSO 인증 등에는 적용되지 않습니다. SSO 인증을 포함한 Fess 애플리케이션 전체(JVM 전체의 HTTP 통신)에 프록시를 사용하려면 환경 변수 ``FESS_PROXY_HOST``, ``FESS_PROXY_PORT`` 를 설정하십시오. 자세한 내용은 아래의 '환경 변수를 통한 프록시 설정'을 참조하십시오.
...
.. note::
   ``fess_config.properties`` 의 ``http.proxy.*`` 설정은 크롤러의 기본 프록시뿐만 아니라 LLM 연동과 플러그인 다운로드에도 사용됩니다.
   SSO 인증을 포함한 Fess 애플리케이션 전체(JVM 전체의 HTTP 통신)에 프록시를 사용하려면 위의 환경 변수를 설정하십시오.
...
OpenSearch 연결 URL과 상태 확인(하트비트) 간격을 설정할 수 있습니다.
...
가상 호스트는 관리 화면의 '시스템 > 일반'(``virtual.host.headers``)에서 설정합니다.
한 줄에 하나씩 ``Host:호스트명[:포트번호]=가상호스트명`` 형식으로 지정합니다.
기본값은 비어 있으며(미설정) 가상 호스트 기능은 비활성화되어 있습니다.
...
.. note::
   ``virtual.host.headers`` 의 값은 ``X-Forwarded-Host,Host`` 와 같은 헤더 이름 목록이 아닙니다.
   각 줄은 ``헤더명:헤더값=가상호스트명`` 형식의 매핑을 기술합니다.
   자세한 형식과 제약은 :doc:`security-virtual-host` 를 참조하십시오.
