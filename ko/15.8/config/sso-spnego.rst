...
.. note::
   ``spnego.allow.delegation`` 을 ``true`` 로 설정하면 SPNEGO 라이브러리는 클라이언트가 위임한
   Kerberos 자격 증명을 받아 인증된 주체에 연결합니다. 다만 현재 |Fess| 는 이 자격 증명을
   어디에서도 사용하지 않으며, 크롤링과 검색, LDAP 조회는 모두 사용자 이름만 사용합니다.
   이 설정은 SPNEGO 핸드셰이크 자체에도 영향을 주지 않습니다. 서버 측(acceptor)의 자격 증명과
   GSS 컨텍스트 플래그는 변하지 않으며, 자격 증명을 위임할지 여부는 클라이언트 측(브라우저 설정과
   Active Directory에서 해당 계정이 위임에 대해 신뢰되는지 여부)에서만 결정됩니다.
   기본값인 ``false`` 로 두십시오. 활성화하면 인증된 요청마다 JDK가 제한된 위임을 시도하는 처리가
   늘어날 뿐 이점은 없습니다.
