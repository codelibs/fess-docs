...
::
    ...
    # 웹 요청의 CPU 사용률 임계값(%)
    # OpenSearch의 CPU 사용률이 이 값 이상일 때 요청을 거부
...
.. note::
   ...
   연속 실패가 3회까지는 WARNING 레벨로 로그에 기록되고, 4회째부터는 DEBUG 레벨로 변경됩니다
   (지속적인 실패로 인한 로그 비대화를 방지하기 위함입니다). 모니터링이 한 번이라도 성공하면 실패 횟수가 리셋됩니다.
...
.. note::
   요청이 거부되면 서버 로그에 INFO 레벨로
   ``Rejecting request due to high CPU load: path=..., cpu=...%, threshold=...%``
   가 출력됩니다. 어떤 경로가 어떤 임계값으로 거부되었는지 확인할 수 있습니다.
...
적응형 부하 제어는 Fess 서버 자체의 시스템 CPU 사용률에 기반하여
백그라운드 작업의 처리 속도를 자동으로 조정하는 기능입니다.
...
.. note::
   적응형 부하 제어는 기본적으로 활성화되어 있습니다 (임계값=50).
   HTTP 요청 부하 제어 (``web.load.control`` / ``api.load.control``)와는 독립적으로 동작합니다.
...
.. list-table::
   ...
   * - 모니터링 대상
     - OpenSearch CPU 사용률
     - Fess 서버의 시스템 CPU 사용률
   * - 제어 대상
     - HTTP 요청 (웹 / API)
     - 백그라운드 작업
   * - 제어 방법
     - HTTP 429를 반환하여 요청 거부
     - 처리 스레드를 일시 대기
   * - 기본값
     - 비활성화 (임계값=100)
     - 활성화 (임계값=50)
...
1. ``adaptive.load.control`` 값을 올리기 (예: 70)
2. Fess 서버의 CPU 리소스 증강
3. 0으로 설정하여 적응형 부하 제어를 비활성화 (권장하지 않음)
