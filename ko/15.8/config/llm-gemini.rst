=====================================
Google Gemini 설정 (AI 검색 / RAG)
=====================================

개요
====

이 페이지에서는 |Fess|\ 가 Google Gemini를 **AI 검색 모드(RAG: Retrieval-Augmented Generation)** — 검색 인덱스를 기반으로 자연어 질문에 출처를 인용하여 답변하는 기능 — 에 사용할 수 있도록 ``fess-llm-gemini`` 플러그인을 설정하는 방법을 설명합니다. |Fess|\ 는 Google AI API(Generative Language API)를 호출하여 Gemini 모델로 크롤링한 문서에 대해 RAG를 수행합니다.
...
.. note::
   이용 가능한 모델의 최신 정보는 `Google AI for Developers <https://ai.google.dev/models/gemini>`__\ 에서 확인할 수 있습니다.
...
2. **Google AI Studio 액세스**: `https://aistudio.google.com/ <https://aistudio.google.com/>`__\ 에 액세스
3. **API 키**: Google AI Studio에서 API 키 생성

API 키 발급
-----------

1. `Google AI Studio <https://aistudio.google.com/>`__\ 에 액세스
2. "Get API key" 클릭
3. "Create API key" 선택
4. 프로젝트 선택 또는 새로 생성
...
Gemini 연계 기능은 ``fess-llm-gemini`` 플러그인으로 제공됩니다.
Gemini를 사용하려면 플러그인 설치가 필요합니다.

1. `fess-llm-gemini-15.8.0.jar` 를 다운로드합니다
...
LLM 프로바이더 선택(``rag.llm.name``)은 관리 화면 또는 ``system.properties``\ 에서 수행하고, AI 검색 모드 기능 활성화 및 Gemini 고유 설정은 ``fess_config.properties``\ 에서 수행합니다.
...
LLM 프로바이더 이름(``rag.llm.name``)은 관리 화면(관리 화면 > 시스템 > 전반)에서 설정하거나 ``system.properties``\ 에 설정합니다. Gemini 고유의 설정은 ``fess_config.properties``\ 에 기술합니다.
...
Gemini 클라이언트에서 사용 가능한 모든 설정 항목입니다. ``rag.llm.name`` 이외는 모두 ``fess_config.properties``\ 에 설정합니다.
...
API 키는 ``x-goog-api-key`` HTTP 요청 헤더로 전송됩니다(Google 권장 방식).
...
보안상의 이유로, API 키는 파일이 아닌 환경(JVM 옵션) 경유로 설정하는 것을 권장합니다.

Docker 환경
-----------

|Fess| 공식의 `docker-fess <https://github.com/codelibs/docker-fess>`__\ 에는 Gemini용 오버레이 ``compose-gemini.yaml``\ 이 동봉되어 있습니다. 최소 절차는 다음과 같습니다.
...
- ``FESS_PLUGINS=fess-llm-gemini:15.8.0``\ 으로 ``run.sh``\ 가 플러그인 JAR을 자동 취득하여 ``app/WEB-INF/plugin/``\ 에 배치합니다
...
- ``-Dfess.system.rag.llm.name=gemini``\ 는 OpenSearch에 값이 아직 기록되지 않은 첫 시작 시에만 기본값으로 적용됩니다. 시작 후에는 관리 화면 "시스템 > 전반"의 RAG 섹션에서도 변경할 수 있습니다

프록시 경유로 인터넷에 접속하는 경우, |Fess|\ 의 ``http.proxy.*`` 설정을 ``FESS_JAVA_OPTS`` 경유로 지정하세요(후술하는 "HTTP 프록시 경유 사용" 참조).
...
Gemini 클라이언트는 |Fess| 전체의 HTTP 프록시 설정을 공유합니다. ``fess_config.properties``\ 에서 다음 프로퍼티를 지정하세요.
...
.. note::
   최신 가격 및 무료 할당량 정보는 `Google AI Pricing <https://ai.google.dev/pricing>`__\ 에서 확인하세요.
