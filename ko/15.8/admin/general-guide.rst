...
IdP(Identity Provider)의 엔티티 ID를 지정합니다.

IdP SSO URL
:::::::::::

IdP의 싱글 사인온 서비스 URL을 지정합니다.

IdP SLO URL
:::::::::::

IdP의 싱글 로그아웃 서비스 URL을 지정합니다.

IdP X.509 인증서
::::::::::::::::

IdP의 SAML 어설션 서명 검증에 사용하는 X.509 공개 키 인증서를 지정합니다.
``-----BEGIN CERTIFICATE-----`` 및 ``-----END CERTIFICATE-----`` 줄을 포함하지 않고 Base64 인코딩된 내용만 지정합니다.
...
SP(Service Provider)의 엔티티 ID를 지정합니다.
``SP 베이스 URL`` 을 설정한 경우 자동으로 설정됩니다.

SP ACS URL
::::::::::

SAML 어설션을 수신하는 Assertion Consumer Service(ACS) URL을 지정합니다.
``SP 베이스 URL`` 을 설정한 경우 자동으로 설정됩니다.

SP SLO URL
::::::::::

SP의 싱글 로그아웃 서비스 URL을 지정합니다.
``SP 베이스 URL`` 을 설정한 경우 자동으로 설정됩니다.

SP NameID 형식
::::::::::::::

SAML 어설션에서 사용하는 NameID 형식을 지정합니다.
...
Microsoft Entra ID의 테넌트 ID 또는 도메인을 지정합니다.
