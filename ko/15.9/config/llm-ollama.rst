...
1. `fess-llm-ollama-15.9.0.jar` 를 다운로드합니다.
...
- ``FESS_PLUGINS=fess-llm-ollama:15.9.0`` 으로 시작 스크립트가 플러그인 JAR을 자동으로 가져와 ``app/WEB-INF/plugin/`` 에 배치합니다(버전은 사용 중인 |Fess| 에 맞게 조정하세요)
