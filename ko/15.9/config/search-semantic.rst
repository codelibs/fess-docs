...
|Fess| 15.9\ 에서는 문서 본문을 청크(조각)로 분할하고 각 청크의 임베딩 벡터를 생성·저장하는
...
- **k-NN 플러그인이 포함된 OpenSearch**: |Fess| 15.9\ 에서는 콘텐츠 청킹 기능의 활성화 여부와
...
.. list-table::
   ...
   * - ``content_chunker.max_chunks_per_document``
     - ``1000``
     - 문서당 최대 청크 수. 이를 초과하는 문서는 ``skipped`` 로 표시되며 임베딩도 생성되지
       않습니다. 경계 인식 분할은 청크를 짧게 만들기 때문에 고정 길이 분할보다 약 3~25% 더 많은
...
.. note::
   ...
   범위 안에 존재하는 가장 높은 계층의 후보 중 가장 가까운 것으로 이동합니다. 줄바꿈·문장 끝은
   아무리 멀리 앞쪽에 있어도 절 구분자·공백보다 우선하며, 이들은 다시 문자 체계 변화보다
   우선합니다. 이동하는 것은 절단 위치뿐이며 문자는 손실되지 않으므로, 문서의 청크를 이어 붙이면
   원본 내용과 정확히 일치합니다. 전방 탐색으로 인해 ``chunk_size`` 를 최대
   ``content_chunker.length.boundary.lookahead_percent`` 만큼 초과할 수 있습니다. 이와는 별도로,
   절단 위치가 자소 클러스터(결합 문자, 이형 선택자, 너비 없는 접합자로 연결된 이모지 시퀀스 등)
   내부에 놓이게 될 경우 최대 32자까지 추가로 초과할 수 있습니다. 이 초과는
   ``lookahead_percent`` 의 영향을 받지 않으며 ``0`` 으로 설정해도 발생할 수 있습니다. 두 종류의
   초과는 동일한 절단 위치에서 동시에 발생하지 않으므로, 기본값 기준 청크 길이는 640~840자
   범위에 들어갑니다. 청크가 평균적으로 짧아지는 만큼 문서에서 생성되는 청크 수는 고정 길이
   분할보다 약 3~25% 늘어납니다(``content_chunker.max_chunks_per_document`` 참고).
...
:doc:`rank-fusion` 을 참조하세요). 융합은 기본적으로 |Fess|\ 가 수행하지만,
``rank.fusion.engine.enabled=true`` 를 설정하면 OpenSearch 측에서 수행할 수도 있습니다
(:ref:`rank-fusion-engine` 을 참조하세요).
...
|Fess| 15.9\ 에서 새로 생성되는 ``fess.search`` 인덱스는 ``content_chunker.search.enabled`` 값과
...
.. note::
   ...
   인덱스에서는 코사인 유사도를 정의할 수 없으므로, 컷오프는 스킵되며 해당 검색마다 경고
   로그가 출력됩니다.

제한 사항
----------

- 쿼리에 포함된 필드 지정 조건(``label:``\ , ``site:``\ , ``filetype:``\ , ``timestamp`` /
  ``last_modified`` 의 범위 지정 등)은 임베딩 대상에서 분리되어 벡터 검색에 필터로 적용됩니다.
  판정은 쿼리를 조립한 **뒤**\ 의 문자열에 대해 이루어지므로, 라벨 지정, 패싯을 이용한 좁히기,
  상세 검색의 파일 종류·사이트·일시 지정 등 내부적으로 이러한 조건이 부가되는 조작(``ex_q`` 나
  ``fields.*`` 파라미터에 의한 것을 포함)에서도 시맨틱 검색이 실행되며, 키워드 검색과 동일한
  조건으로 좁혀집니다.
- 다음 쿼리에서는 **시맨틱 검색이 스킵**\ 되고 키워드 검색만 실행됩니다.

  - 정렬 조건 지정(``sort:`` 의사 필드 또는 ``sort`` 파라미터. 후자도 내부적으로
    ``sort:...`` 로 부가됩니다)
  - 필드를 지정하지 않은 단어에 대한 구문(``"..."``), 와일드카드(``*`` ``?``), 퍼지 검색(``~``),
    범위 지정. 상세 검색의 구문 지정도 포함합니다. 반각 ``?`` 는 와일드카드로 해석되므로,
    "~란?"처럼 반각 물음표로 끝나는 자연문도 스킵됩니다(전각 ``？`` 는 대상이 아닙니다).
  - 단어 제외(``-단어``\ , ``NOT 단어``\ . 상세 검색의 제외어 포함)나 단어 부스트(``단어^2``)
  - 단어와 조건을 ``OR`` 로 연결한 쿼리 등, 조건을 필터로 분리할 수 없는 쿼리
  - ``allintitle:`` / ``allinurl:``
  - 조건만 있고 임베딩 대상이 되는 단어를 포함하지 않는 쿼리
  - 구문 분석할 수 없는 쿼리
...
.. note::
   ...
   벡터 검색을 사용하지 않더라도 |Fess| 15.9 이후에서 인덱스를 **신규 생성**\ (재인덱싱 포함)하면
...
|Fess| 15.7 이전에서 시맨틱 검색을 제공하던 ``fess-webapp-semantic-search`` 플러그인은 15.9에서
