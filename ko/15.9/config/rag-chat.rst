...
|Fess| 15.9에서는 LLM 기능이 ``fess-llm-*`` 플러그인으로 분리되었습니다.
...
|Fess| 15.9에서는 설정이 두 가지 계열로 분리되어 있습니다: FessConfig 계열
...
.. note::
   ...
   15.9 이후에는 ``rag.llm.name`` 으로 변경되었으며, ``rag.llm.type`` 에 작성된 값은
   읽혀지지 않습니다.
...
|Fess| 15.9에서는 생성 파라미터(최대 토큰 수, temperature 등)를 프로바이더별,
...
|Fess| 15.9에서는 시스템 프롬프트가 프로퍼티 파일이 아닌 각 ``fess-llm-*``
...
.. note::
   ...
   15.9에서는 ``/api/v2/`` 하위의 JSON 기반 API를 사용하세요.
