...
웹 크롤러를 통한 웹사이트 검색, 파일 크롤러를 통한 파일 서버 검색, 데이터 스토어 크롤러를 통한 데이터베이스 등 다양한 데이터 소스 검색까지, 하나의 |Fess| 로 횡단적인 전문 검색을 구축할 수 있습니다.
본 가이드에서는 크롤러의 기본 개념과 설정 방법에 대해 설명합니다.

데이터베이스 등 기타 데이터 소스 크롤링에 대해서는 :doc:`datastore/ds-database` 와 :doc:`datastore/ds-overview` 를 참조하십시오. 크롤링한 콘텐츠 전반의 관련도를 하이브리드(키워드 + 시맨틱) 검색으로 향상시키려면 :doc:`rank-fusion` 을 참조하십시오.
...
- :doc:`../admin/webconfig-guide` - 웹 크롤링(웹사이트 검색) 설정
- :doc:`../admin/fileconfig-guide` - 파일 크롤링(파일 서버 검색) 설정
- :doc:`../admin/dataconfig-guide` - 데이터 저장소 크롤링(데이터베이스 검색) 설정
- :doc:`datastore/index` - 데이터스토어 커넥터 가이드
- :doc:`search-basic` - 검색 기능
- :doc:`datastore/ds-overview` - 데이터 스토어 크롤링 개요(데이터베이스 등 기타 데이터 소스)
- :doc:`datastore/ds-database` - 데이터베이스 크롤링 설정
- :doc:`rank-fusion` - 하이브리드 검색과 Rank Fusion
