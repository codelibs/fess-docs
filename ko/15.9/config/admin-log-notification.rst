...
.. important::
   실패한 스케줄 잡(잡 로그에 상태 ``fail`` 로 기록됨)은 ``ERROR`` 가 아니라
   ``WARN`` ( ``Failed to execute job: ...`` )으로 로그에 기록됩니다. 따라서 레벨이 ``ERROR`` 이면
   실패한 스케줄 잡은 **통지되지 않습니다**. 이를 통지받으려면 레벨을 ``WARN`` 으로
   설정하십시오. 이 경우 그 밖의 모든 ``WARN`` 이벤트도 통지됩니다.
...
1. 메일 서버 설정:
   ...
   이 값은 ``fess_env_web.properties`` 에 설정합니다. 이 파일은 로그 통지와 **일반** 설정의 테스트
   메일을 전송하는 |Fess| 웹 프로세스가 읽습니다. 크롤러는 ``fess_env_crawler.properties`` 를 읽는
   별도의 프로세스로 실행되며 크롤 완료 메일을 전송하므로, 이 파일에도 같은 값을 설정하십시오.
   파일은 ZIP 패키지의 경우 ``app/WEB-INF/classes/`` 에, RPM/DEB 패키지의 경우 ``/etc/fess/`` 에
   있습니다. ``bin/fess`` 는 ``-Dlasta.env=web`` 으로 |Fess| 를 시작하므로, ``fess_env.properties``
   에만 설정한 값은 사용되지 않습니다. 변경 후 |Fess| 를 재시작하십시오.
...
.. list-table::
   ...
   * - 운영 환경
     - ``ERROR``
     - 중요한 오류만 통지하여 노이즈를 줄임. 이 레벨에서는 실패한 스케줄 잡이 통지되지 않으므로,
       필요하면 ``WARN`` 을 사용
   * - 스테이징 환경
     - ``WARN``
     - 잠재적인 문제도 포함하여 통지
...
3. **메일 서버 설정 확인**
   ...
   메일 통지를 사용하는 경우, ``fess_env_web.properties`` 에서 메일 서버가 올바르게
   설정되어 있는지 확인합니다(「설정」의 「메일 통지」 참조). **일반** 설정의 [테스트 메일 보내기]
   버튼은 같은 설정을 통해 메일을 전송합니다.
