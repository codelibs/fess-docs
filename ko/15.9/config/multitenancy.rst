...
1. 가상 호스트 키를 소문자로 한 이름의 정적 테마를 만듭니다. 예: ``tenant1``
   ( :ref:`theme-customize-bundled` 참조)
2. "시스템" → "테마"에서 설치합니다

그러면 해당 가상 호스트에 대한 요청에 그 테마가 사용됩니다. 자세한 내용은 :doc:`security-virtual-host` 를
참조하세요.

커스텀 CSS
----------

가상 호스트별로 커스텀 CSS를 적용하려면 해당 가상 호스트 테마의 ``assets/styles.css`` 를 변경합니다.
