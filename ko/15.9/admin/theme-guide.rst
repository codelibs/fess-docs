...
검색 화면은 항상 정적 테마로 표시됩니다. 기본 테마를 설정하지 않은 경우에는 |Fess| 에 번들된 정적 테마 ``bootstrap`` 이 사용됩니다. 번들된 테마는 삭제하거나 교체할 수 없습니다. 변경하려면 다른 이름으로 복사하십시오( :ref:`theme-customize-bundled` 참조).

.. note::
   JSP 기반 테마(JAR 테마)는 플러그인 관리에서 취급하므로, 이 페이지의 대상 외입니다. 15.9 부터는 JAR 테마로 검색 화면을 변경할 수 없습니다.
...
* 이 페이지에서 저장소로부터 설치합니다. 아래의 `저장소에서 설치`_ 절차를 참조하십시오.
...
목록 페이지 상단의 풀다운 메뉴에서 테마를 선택하고 [기본값으로 설정] 버튼을 클릭하면 검색 화면에 적용할 기본 테마를 설정할 수 있습니다. [(기본값 없음)]을 선택하여 설정하면 기본 테마 지정이 해제되고, 번들된 ``bootstrap`` 테마가 다시 사용됩니다. 설정 후에는 테마 정보가 다시 로드되어 즉시 반영됩니다.


저장소에서 설치
---------------

목록 페이지에서는 테마 저장소( ``theme.repositories`` , 기본값은 https://maven.codelibs.org/release/org/codelibs/fess/themes/ )에 공개된 테마도 설치할 수 있습니다.

* [사용 가능한 테마] 에는 저장소가 공개하는 테마가 각각의 버전과 함께 나열됩니다. 행의 [설치] 를 클릭하면 해당 버전이 설치됩니다.
* [이름으로 설치] 에서는 목록에 표시되는지 여부와 관계없이 이름과 버전을 지정하여 테마를 설치합니다. [이름] 과 [버전] 을 입력하고 [설치] 를 클릭합니다.

ZIP 은 저장소에서 내려받아 공개된 ``.sha1`` 체크섬과 대조한 뒤, 업로드와 같은 방법으로 설치됩니다. 따라서 같은 검사가 적용됩니다. 사용하려면 설치 후 기본 테마로 설정하십시오.
...
.. list-table::
   ...
   * - ``spaFallback``
     - 선택
     - 더 이상 사용되지 않으며 읽히지 않습니다. 15.9 부터는 검색 화면의 각 경로에서 항상 엔트리 포인트 파일이 반환됩니다.
