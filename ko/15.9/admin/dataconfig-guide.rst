...
데이터 저장소에 관한 파라미터를 지정합니다.

``script_type`` 파라미터로 아래 스크립트를 평가할 스크립트 엔진을 지정합니다.
신규 작성 시에는 ``script_type=javascript`` 가 미리 입력되어 있습니다.
``script_type`` 을 지정하지 않으면 스크립트는 Groovy로 평가됩니다.
...
값 측은 ``script_type`` 파라미터로 지정한 스크립트 엔진의 구문으로 작성합니다.
...
.. |image0| image:: ../../../resources/images/en/15.9/admin/dataconfig-1.png
.. |image1| image:: ../../../resources/images/en/15.9/admin/dataconfig-2.png
