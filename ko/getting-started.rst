...
.. note::
   ...
   Fess로 검색하려면 먼저 크롤을 설정·실행하여 인덱스를 구축해야 합니다.
   설치 직후에는 검색 결과가 0건이 됩니다.
   크롤 설정 방법은 `관리 화면(관리자용)`_ 을 참조하세요.
...
예: ``"전문 검색"`` → "전문 검색"이라는 구문(연속된 어구)을 포함하는 문서
...
.. warning::
   ...
   **보안에 관한 중요한 주의**
   ...
   비밀번호 변경 방법은 :doc:`15.6/admin/user-guide` 를 참조하세요.
...
- :doc:`15.6/admin/index` — 크롤 설정, 검색 설정 상세
- :doc:`15.6/api/index` — REST API를 사용한 검색 통합
