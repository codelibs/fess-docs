...
웹사이트나 파일 서버, 데이터베이스 등 사내에 흩어져 있는 문서를 횡단적으로 전문 검색할 수 있는 오픈소스 엔터프라이즈 서치(사내 검색) 소프트웨어입니다.

OpenSearch를 검색 엔진으로 사용하며, 키워드 검색·패싯·역할 기반 검색을 기본으로 제공합니다. 선택적 플러그인을 통해 **시맨틱(벡터) 검색**\ , **하이브리드 검색**\ , 그리고 Ollama·OpenAI·Google Gemini 등의 LLM을 활용한 **AI 검색 모드(RAG)**\ 를 추가할 수 있습니다.
...
- :doc:`Fess 15.8.0 <downloads>` (zip/rpm/deb 패키지)
...
-  웹, 파일 시스템, Windows 공유 폴더, 데이터베이스를 크롤 (웹, 파일 서버, 데이터베이스 크롤링)
...
-  Ollama·OpenAI·Google Gemini 등의 LLM과 연계한 AI 검색 모드(RAG: Retrieval-Augmented Generation) (``fess-llm-*`` 플러그인 경유)

-  Semantic Search 플러그인을 통한 키워드(BM25)와 시맨틱(벡터) 검색을 결합한 하이브리드 검색
