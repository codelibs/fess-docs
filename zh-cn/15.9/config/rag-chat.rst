...
|Fess| 15.9 中，LLM功能已作为 ``fess-llm-*`` 插件分离提供。
...
|Fess| 15.9 中，配置项分为两个系列：FessConfig 系列（ ``fess_config.properties`` ）和
...
.. note::
   ...
   ``rag.llm.type`` 是 |Fess| 15.5 及以前的旧属性名。15.9 及以后已改为 ``rag.llm.name``，
...
|Fess| 15.9 中，生成参数（最大token数、temperature等）按提供商
...
|Fess| 15.9 中，系统提示词不在属性文件中定义，而是在各 ``fess-llm-*``
...
.. note::
   ...
   15.9 中请使用 ``/api/v2/`` 下基于 JSON 的 API。
