...
.. important::
   执行失败的调度任务(在任务日志中以状态 ``fail`` 记录)是以 ``WARN``
   (``Failed to execute job: ...``)而不是 ``ERROR`` 级别输出日志的。因此,级别设置为 ``ERROR`` 时,
   失败的调度任务 **不会被通知**\ 。要接收这些通知,请将级别设置为 ``WARN``\ ;此时其他所有
   ``WARN`` 事件也都会被通知。
...
1. 邮件服务器设置:
   ...
   请在 ``fess_env_web.properties`` 中设置此项。该文件由发送日志通知和 **通用** 设置中测试邮件的
   |Fess| Web 进程读取。爬虫作为单独的进程运行,读取 ``fess_env_crawler.properties`` 并发送爬取完成
   邮件,因此也请在该文件中设置相同的值。ZIP 包的这些文件位于 ``app/WEB-INF/classes/`` 中,RPM/DEB
   包则位于 ``/etc/fess/`` 中。仅在 ``fess_env.properties`` 中设置的值不会被使用,因为 ``bin/fess``
   以 ``-Dlasta.env=web`` 启动 |Fess|\ 。更改后请重启 |Fess|\ 。
...
.. list-table::
   ...
   * - 生产环境
     - ``ERROR``
     - 仅通知重要错误,减少噪音。此级别下不会通知失败的调度任务;如需通知,请使用 ``WARN``
   * - 演示环境
     - ``WARN``
     - 包含潜在问题一并通知
   * - 开发环境
...
3. **确认邮件服务器设置**
   ...
   使用邮件通知时,确认 ``fess_env_web.properties`` 中邮件服务器是否正确配置(参见"设置"中的"邮件通知")。
   **通用** 设置中的 [发送测试邮件] 按钮会使用相同的配置发送邮件。
