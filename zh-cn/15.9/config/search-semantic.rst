...
在 |Fess| 15.9 中，**内容分块功能**\ ——将文档正文拆分为若干分块，并为每个分块生成并存储嵌入
...
- **带 k-NN 插件的 OpenSearch**：在 |Fess| 15.9 中，无论内容分块功能是否启用，搜索索引
...
默认情况下由 |Fess| 自身执行融合；设置 ``rank.fusion.engine.enabled=true`` 后，改由 OpenSearch
执行融合（请参阅 :ref:`rank-fusion-engine`\ ）。
...
在 |Fess| 15.9 下新创建的任何 ``fess.search`` 索引，无论 ``content_chunker.search.enabled`` 的
...
.. note::
   ...
   该阈值会被跳过，并且每次这样的搜索都会输出警告日志。

限制事项
--------

- 查询中带字段限定的条件（``label:``、``site:``、``filetype:``、``timestamp`` / ``last_modified``
  的范围等）会从用于嵌入的文本中分离出来，并作为过滤条件应用于向量搜索。由于查询是在组装
  **之后**\ 才被分析的，因此，即使是在内部附加此类条件的操作——指定标签、通过分面进行筛选，
  以及高级搜索中的文件类型、站点指定、日期时间指定，包括通过 ``ex_q`` 和 ``fields.*`` 参数传入的
  条件——也仍会执行语义搜索，并按与关键词搜索相同的条件进行筛选。
- 对于以下查询，\ **将跳过语义搜索**\ ，仅执行关键词搜索。

  - 指定排序条件（``sort:`` 伪字段，或 ``sort`` 参数，后者在内部同样会以 ``sort:...`` 的形式附加）
  - 针对未指定字段的词语的短语（``"..."``）、通配符（``*`` ``?``）、模糊搜索（``~``）或范围，
    包括高级搜索中的短语。半角的 ``?`` 会被视为通配符，因此像“……是什么?”这样以半角问号结尾的
    自然语句也会被跳过（全角的 ``？`` 不在范围内）。
  - 排除词语（``-word``、``NOT word``\ ，包括高级搜索中的排除词）或提升权重（``word^2``）
  - 无法将条件作为过滤条件分离出来的查询，例如用 ``OR`` 连接词语与条件的查询
  - ``allintitle:`` / ``allinurl:``
  - 只有条件、没有可用于嵌入的词语的查询
  - 无法解析的查询
...
.. note::
   ...
   即使您不使用向量搜索，只要在 |Fess| 15.9 及以后\ **新建**\ 索引（包括重新索引），就会应用
...
15.9 中并入核心，现已\ **不再需要（已弃用）**\ 。除了上文\ *现有用户且想启用向量搜索*\ 中的步骤外，
您还需要执行以下操作。
