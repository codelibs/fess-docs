...
.. note::
   SAML认证的支持由 ``fess-sso-saml`` 插件提供，该插件不随发行包分发。请从管理界面的
   「系统 > 插件」页面安装，或执行 ``bin/fess-setup install plugin fess-sso-saml`` 。
   在安装之前，即使设置 ``sso.type=saml``\ ，对 ``/sso/`` 的请求也只会重定向回登录页面。
