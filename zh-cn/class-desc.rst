...
* https://github.com/codelibs/fess/blob/master/src/main/java/org/codelibs/fess/exec/Crawler.java
 ...
 - toString
  - 返回会话ID、名称、Web配置ID、文件配置ID、数据配置ID、属性路径、删除期限的字符串
 ...
 - main方法
  ...
  - 设置系统属性
    ...
    - 集群名称
  - try
    - 生成单例La容器工厂 //TODO 调查la容器
      - 设置配置路径(app.xml)
      - 设置外部上下文 //TODO 调查外部上下文
      - 设置外部上下文Def注册器
    - 生成用于关闭的线程
    - 将进程的返回值放入退出代码
  - catch
 ...
 - 方法：处理
   - 生成爬虫
   - 选项：如果有会话ID则设置(以yyyyMMddHHmmss格式设置日期)
   - 生成爬取信息助手
   - ダイナミックプロパティを生成する(?)
   - try
     - 选项：有属性路径
     ...
     - 选项：无属性路径
       ...
       - 删除临时文件。
       - 使用临时文件更新系统属性
       - deleteOnExit //TODO :L249 删除属性文件的退出代码?
   - catch
     - 输出文件输入输出错误的日志
 ...
 - 方法：执行爬取
   - try
     ...
     - 将会话ID和路径映射信息设置到路径映射助手
     - duplicateHostHelper初始化
     - duplicateHostHelper // TODO 调查duplicateHostHelper是什么

     - deleteSessionIdsBefore // TODO 删除与会话ID关联的删除期限？


     - 如果有Web文件配置ID列表，则生成Web文件爬取的线程
     - 如果有Web文件配置ID列表，则生成数据爬取的线程
 ...
 - 方法：输出会话信息时间
   - クロール情報に時間を设置(yyyy-MM-dd'T'HH:mm:ss.SSSZ)
