...
.. list-table::
   ...
   * - ``issue.jql``
     - 否
     - JQL（仅Jira，高级搜索条件）。未指定时，以所有问题（``created is not empty``）为处理对象。
   * - ``issue_max_results``
     - 否
...
.. note::
   Confluence连接器会同时获取普通页面（page）和博客文章（blogpost）。
...
令牌更新后，新的访问令牌和刷新令牌将自动保存回数据存储配置中，后续抓取将使用更新后的令牌，无需手动更新。
