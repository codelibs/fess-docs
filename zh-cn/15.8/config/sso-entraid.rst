...
- |Fess| 会在登录完成后于后台解析用户所属的组和角色，因此登录本身不会等待Microsoft Graph的响应。
  在解析完成之前，用户仅缺少与组、角色相关联的权限——用户自身的用户级权限，以及在
  ``entraid.default.groups`` 和 ``entraid.default.roles`` 中配置的组和角色，从第一个请求起即已
  存在——因此本应可以查看的文档可能暂时不会出现在搜索结果中。解析进行期间，搜索界面会显示相应提示
- 如果解析失败，搜索界面会显示提示；如果问题反复出现，请联系管理员。但失败并不一定就是最终结果：
  每次刷新访问令牌时都会重新解析，之后一旦成功，提示即会消失，缺少的权限也会恢复。若要立即重试，
  用户需要先注销再重新登录——在保持登录状态下打开SSO登录URL，只会重定向回搜索界面
