...
::
    ...
    # Web请求的CPU使用率阈值（%）
    # 当OpenSearch的CPU使用率达到或超过此值时拒绝请求
    # 设置为100则禁用（默认: 100）
...
.. note::
   ...
   连续失败前3次以WARNING级别记录日志，第4次起改为DEBUG级别
   （防止因持续失败导致日志膨胀）。监控只要成功一次，失败计数即重置。
