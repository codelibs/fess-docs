...
``q`` 参数可以使用与常规搜索界面相同的搜索语法，包括 AND/OR/NOT 检索、字段指定检索、通配符检索、模糊检索等。搜索语法的详细说明请参阅 :doc:`../user/index`。
...
.. list-table:: 请求参数
   ...
   * - ``num``
     - 每页条数（integer，\ ``>=1``\ ，默认值 ``10``\ ）。\ ``<= 0`` 会返回 ``invalid_request``\ 。超过配置最大值的数值会被静默截断。是否被截断可通过比较请求中的 ``num`` 与响应中的 ``page_size`` 来判断。
   * - ``sort``
     - 排序方式（例：\ ``score``\ ）。
   * - ``lang``
     - 搜索语言。可重复指定（数组）。例：\ ``en``\ 。
...
- :doc:`../user/search-field` - 字段指定检索
- :doc:`../user/special-char` - 特殊字符与转义
- :doc:`../user/search-wildcard` - 通配符检索
