...
|Fess| 15.7中，Gemini集成功能以 ``fess-llm-gemini`` 插件的形式提供。
使用Gemini需要安装插件。
...
|Fess| 15.7中，AI搜索模式功能的启用及Gemini专属配置在 ``fess_config.properties`` 中进行，LLM提供商名称（ ``rag.llm.name`` ）的选择在管理界面或 ``system.properties`` 中进行。
