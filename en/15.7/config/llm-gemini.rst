...
In |Fess| 15.7, Gemini integration is provided as the ``fess-llm-gemini`` plugin.
To use Gemini, you must install the plugin.

1. Download `fess-llm-gemini-15.7.0.jar`
...
In |Fess| 15.7, enabling AI mode functionality and Gemini-specific settings are done in ``fess_config.properties``, while selecting the LLM provider is done from the administration screen or in ``system.properties``.
