...
In |Fess| 15.9, LLM functionality has been separated as ``fess-llm-*`` plugins.
...
In |Fess| 15.9, settings are split into two families: the FessConfig family
...
.. note::
   ...
   In 15.9 and later it has been renamed to ``rag.llm.name``; values written
...
In |Fess| 15.9, generation parameters (maximum tokens, temperature, etc.) are configured
...
In |Fess| 15.9, system prompts are defined in the DI XML (``fess_llm++.xml``) of each ``fess-llm-*``
...
.. note::
   ...
   Use the ``/api/v2/`` JSON-based API in 15.9.
