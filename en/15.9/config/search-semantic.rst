...
In |Fess| 15.9, the **content chunking feature** — which splits document bodies into chunks and
...
- **OpenSearch with the k-NN plugin**: In |Fess| 15.9, the mapping for the search index
...
Any ``fess.search`` index newly created under |Fess| 15.9 always has ``index.knn`` and ANN method
...
.. note::
   ...
   Even if you never use vector search, **creating a new index** under |Fess| 15.9 or later
...
earlier, has been folded into the core in 15.9 and is now **unnecessary (deprecated)**. In
