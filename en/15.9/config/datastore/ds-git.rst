...
.. list-table::
   ...
   * - ``readInterval``
     - No
     - Processing interval between each file (in milliseconds, default: ``0``)
...
Instead of embedding credentials in the URI, you can also specify credentials using the ``username`` and ``password`` parameters:
...
Credentials are used only when both ``username`` and ``password`` are specified.

.. note::
   The Git connector supports only HTTP/HTTPS authentication (username and password, or an access token). SSH key authentication (``git@...`` style URIs) is not supported. Use an HTTPS-style URI.
...
When ``base_url`` is set, files detected as deleted via Git DiffEntry (``ChangeType.DELETE``) are automatically removed from the index. When a file is renamed (``ChangeType.RENAME``), the document at the old path is deleted and the file at the new path is re-indexed.

.. note::
   Automatic deletion of removed files is only effective when ``base_url`` is set. If ``base_url`` is not set, the document URL is empty, so deletion is not performed.
...
The URL is generated by simply concatenating ``base_url`` and the file path (no separator is inserted). Therefore, ``base_url`` must end with a trailing ``/``.
