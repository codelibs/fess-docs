...
.. list-table::
   ...
   * - ``fess.json`` / ``doc.json``
     - The index mapping definition file itself (``fess_indices/fess.json`` / ``fess_indices/fess/doc.json``), returned as-is (``application/octet-stream``)
...
.. note::
   ...
   This API only downloads. To restore (upload) backup files, including ``fess.json`` and
   ``doc.json``, use System Info → Backup in the admin UI.
...
.. note::
   ...
   If ``{id}`` contains a dot (``.``), as ``fess_config.bulk``, ``system.properties``, ``fess.json`` and
