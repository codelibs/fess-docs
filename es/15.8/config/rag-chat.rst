...
En |Fess| 15.8, la funcionalidad LLM ha sido separada como plugins ``fess-llm-*``.
...
En |Fess| 15.8 los parametros estan divididos en dos familias: la familia FessConfig
...
.. note::
   ...
   En 15.8 y posteriores se renombro a ``rag.llm.name``; los valores escritos bajo
...
En |Fess| 15.8, los parametros de generacion (numero maximo de tokens, temperature, etc.) se configuran por proveedor
...
En |Fess| 15.8, los prompts del sistema estan definidos en el DI XML (``fess_llm++.xml``) de cada plugin ``fess-llm-*``,
...
.. note::
   ...
   En la version 15.8, utilice la API basada en JSON de ``/api/v2/``.
...
2. ¿Esta instalado el plugin ``fess-llm-*`` correspondiente?
   ...
   - Docker: ``FESS_PLUGINS=fess-llm-gemini:15.8.0`` (o ``fess-llm-openai`` / ``fess-llm-ollama``) debe estar definido
   - Instalacion por paquete: el JAR debe estar en ``app/WEB-INF/plugin/``
   - El log de inicio debe incluir ``Installing fess-llm-XXX-15.8.0.jar``
