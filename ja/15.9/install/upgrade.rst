...
15.8 からアップグレードする場合、以下の 6 点が互換性のない変更です。
...
ファイルクロール設定は不正なプロトコルとして拒否され、ストレージ画面は
``storage.type=gcs`` に対応するクライアントが登録されていないと報告します。

Google Cloud Storage を使用していない場合、対応は不要です。Amazon S3 と MinIO などの
S3 互換ストレージは配布物に残っており、影響はありません。
...
``file,smb,smb1,ftp,s3`` です。代わりに ``s3`` を使用し、 ``storage:`` で始まるパスを
