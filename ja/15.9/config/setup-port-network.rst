...
.. warning::
   ``FESS_PORT`` 環境変数は Linux 環境でのみ有効です。Windows 環境では ``bin\fess.in.bat`` の ``-Dfess.port`` を直接編集してください。
...
Windows環境では、``bin\fess.in.bat`` の ``-Dfess.context.path`` を直接編集してください。
...
.. warning::
   ``FESS_CONTEXT_PATH`` 環境変数は Linux 環境でのみ有効です。Windows 環境では ``bin\fess.in.bat`` を直接編集してください。
...
クローラーのプロキシ設定では、特定ホストの除外（nonProxyHosts）はサポートされていません。
特定のホストをプロキシから除外する場合は、環境変数 ``FESS_NON_PROXY_HOSTS`` を使用してください。
この設定はJavaシステムプロパティ ``http.nonProxyHosts`` に変換され、Fess全体のHTTP通信に適用されます。
...
クロール設定で個別に ``client.proxyHost`` を指定していない場合のデフォルトプロキシを ``fess_config.properties`` で設定できます。
この設定は、クローラーのデフォルトプロキシに加えて、LLM連携のHTTP通信やプラグインのダウンロードにも適用されます。
一方で、SSO認証など一部のJavaライブラリによる通信には適用されません。
Fessアプリケーション全体(SSO認証を含む)でプロキシを適用する場合は、後述の環境変数(``FESS_PROXY_HOST`` 等)を使用してください。
...
.. note::
   ``http.proxy.*`` 設定はSSO認証などには適用されません。SSO認証を含むFessアプリケーション全体（JVM全体のHTTP通信）でプロキシを使用する場合は、環境変数 ``FESS_PROXY_HOST``、``FESS_PROXY_PORT`` を設定してください。詳細は後述の「環境変数によるプロキシ設定」を参照してください。
...
.. note::
   ``fess_config.properties`` の ``http.proxy.*`` 設定は、クローラーのデフォルトプロキシのほか、LLM連携やプラグインのダウンロードに使用されます。
   SSO認証を含むFessアプリケーション全体（JVM全体のHTTP通信）でプロキシを使用する場合は、上記の環境変数を設定してください。
...
OpenSearchへの接続先URLとヘルスチェック間隔を設定できます。
...
仮想ホストは、管理画面の「システム > 全般」(``virtual.host.headers``)で設定します。
1行につき1つの仮想ホストを ``Host:ホスト名[:ポート番号]=仮想ホスト名`` の形式で指定します。
デフォルトは空(未設定)で、仮想ホスト機能は無効です。
...
.. note::
   ``virtual.host.headers`` の値は、``X-Forwarded-Host,Host`` のようなヘッダー名のリストではありません。
   ``ヘッダー名:ヘッダー値=仮想ホスト名`` という対応を1行ごとに記述します。
   詳細な書式や制約は :doc:`security-virtual-host` を参照してください。
