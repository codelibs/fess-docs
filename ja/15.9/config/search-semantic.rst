...
|Fess| 15.9 では、ドキュメント本文をチャンク（断片）に分割し、各チャンクの埋め込みベクトルを
...
- **OpenSearch（k-NN プラグイン入り）**: |Fess| 15.9 では、コンテンツチャンク機能を有効にしているか
...
.. list-table::
   ...
   * - ``content_chunker.max_chunks_per_document``
     - ``1000``
     - 1ドキュメントあたりの最大チャンク数。超過したドキュメントは ``skipped`` になり、
       埋め込みも作成されません。境界考慮分割はチャンクを短くするため、固定長分割より
       およそ3〜25%多くのチャンクが生成されます。非常に大きなドキュメントを扱う場合は
       この値を大きくしてください
   ...
   * - ``content_chunker.search.knn.k``
     - ``100``
     - ANNクエリで取得する近傍数（ページング範囲が大きい場合は自動的に拡大。検索エンジン側で
       融合する場合は ``rank.fusion.pagination_depth`` 以上になります）
...
.. note::
   ...
   探索ウィンドウ内に存在する最上位ティアの候補のうち最も近いものへ移動します。改行・文末は、
   どれだけ手前にあっても節区切りや空白より優先され、それらは書字系の変わり目より優先されます。
   移動するのは分割位置だけで文字が失われることはないため、ドキュメントのチャンクを連結すれば
   元の内容と完全に一致します。前方探索によって、``chunk_size`` を最大
   ``content_chunker.length.boundary.lookahead_percent`` 分だけ超える場合があります。これとは
   別に、書記素クラスター（結合文字、異体字セレクター、ゼロ幅接合子で連結された絵文字
   シーケンスなど）の途中で分割されそうになった場合には、最大32文字の超過が発生することが
   あります。こちらは ``lookahead_percent`` の影響を受けず、``0`` に設定していても発生します。
   2種類の超過が同じ分割位置で同時に発生することはないため、標準設定でのチャンク長は
   640〜840文字の範囲に収まります。平均的にチャンクが短くなる分、ドキュメントから生成される
   チャンク数は固定長分割よりおよそ3〜25%増えます
...
（Rank Fusion の仕組みは :doc:`rank-fusion` を参照）。融合は既定では |Fess| が行いますが、
``rank.fusion.engine.enabled=true`` を設定すると OpenSearch 側で行うこともできます
（:ref:`rank-fusion-engine` を参照）。
...
|Fess| 15.9 で新規作成される ``fess.search`` インデックスは、``content_chunker.search.enabled`` の
...
.. note::
   ...
   モードのインデックスでは、コサイン類似度を定義できないため、足切りはスキップされ、
   該当する検索ごとに警告ログが出力されます。

制限事項
--------

- クエリに含まれるフィールド指定の条件（``label:``\ 、``site:``\ 、``filetype:``\ 、
  ``timestamp`` / ``last_modified`` の範囲指定など）は、埋め込みの対象から切り離され、ベクトル
  検索にフィルタとして適用されます。判定はクエリの組み立て **後** の文字列に対して行われるため、
  ラベルの指定、ファセットによる絞り込み、詳細検索のファイル種別・サイト・日時の指定など、内部的
  にこうした条件が付加される操作（``ex_q`` や ``fields.*`` パラメーターによるものを含む）でも
  セマンティック検索は実行され、キーワード検索と同じ条件で絞り込まれます。
- 次のクエリでは **セマンティック検索はスキップ** され、キーワード検索のみが実行されます。

  - ソート条件の指定（``sort:`` 疑似フィールド、または ``sort`` パラメーター。後者も内部的に
    ``sort:...`` として付加されます）
  - フィールドを指定しない語に対するフレーズ（``"..."``）、ワイルドカード（``*`` ``?``）、
    あいまい検索（``~``）、範囲指定。詳細検索のフレーズ指定も含みます。半角の ``?`` はワイルド
    カードとして解釈されるため、「〜とは?」のように半角疑問符で終わる自然文もスキップされます
    （全角の ``？`` は対象外です）。
  - 語の除外（``-語``\ 、``NOT 語``\ 。詳細検索の除外語を含む）や語のブースト（``語^2``）
  - 語と条件を ``OR`` で結んだクエリなど、条件をフィルタとして切り離せないクエリ
  - ``allintitle:`` / ``allinurl:``
  - 条件だけで、埋め込みの対象となる語を含まないクエリ
  - 構文解析できないクエリ

- 位置情報検索（ジオフィルタ）や類似ドキュメント検索と併用した場合もスキップされます。
- 深いページではRank Fusion自体が無効化されます。

  - |Fess| 側で融合する場合（既定）は、境界は ``rank.fusion.window_size``\ （デフォルト
    ``200``）で決まり、既定では検索結果の 101 件目以降がキーワード検索のみの結果になります。
  - 検索エンジン側で融合する場合（``rank.fusion.engine.enabled=true``）は、境界は
    ``rank.fusion.pagination_depth``\ （デフォルト ``200``）です。開始位置＋ページサイズがこの値を
    超えるページは |Fess| 側での融合で処理され、上記の ``rank.fusion.window_size`` の境界が
    適用されます。

  詳細は :doc:`rank-fusion` を参照してください。
...
.. note::
   ...
   ベクトル検索を利用しない場合でも、|Fess| 15.9 以降でインデックスを **新規作成**\ （再インデ
...
プラグインは、15.9 でコアに統合されたため **不要になりました（非推奨）**。上記
