...
クエリのタイムアウトやシャードの失敗により結果が不完全になった検索を、ログに出力するかどうかの設定です。
有効な場合、以下のときに WARN レベルのログを出力します。

- ``[SEARCH TIMEOUT]`` : 検索時のタイムアウト値 ``query.timeout`` を超え、検索エンジンが結果の収集を打ち切った場合
- ``[SEARCH SHARD FAILURE]`` : 1つ以上のシャードで検索が失敗した場合

両方が発生した場合は、両方のログを出力します。
各ログには ``exec_time`` （ミリ秒）、リクエスト、およびレスポンス全体が含まれます。
シャードの失敗理由は、そのレスポンスの ``_shards.failures`` に記録されています。検索エンジン自体はこの理由を DEBUG レベルでしかログに出力しません。
初期値は ``true`` (有効)です。
::
...
- 数値は数値型のフィールドになります。整数は ``long`` 型、小数は ``float`` 型です。JavaScript
  エンジンはすべての数値を浮動小数点数として渡すので、 ``parseInt`` で変換した値も ``float`` 型に
  なります。
...
数値のフィールドは、そのフィールドを持つ最初の文書を登録する前にインデックスのマッピングに定義して
ください。自動で作られる ``float`` 型の有効桁数は 7 桁程度で、16,777,216 を超える整数は近い値に
丸められることがあります。たとえば ``price`` が ``20000001`` の文書が ``price:[20000000 TO 20000000]`` に一致し、
ソートでも区別されません。整数には ``long`` 型、小数には ``double`` 型を使います。

1. ``fess.update`` エイリアスを通して、現在のドキュメントインデックスにフィールドを追加します。
...
2. 同じ定義を、インストール先の ``app/WEB-INF/classes/fess_indices/fess/doc.json`` の
   ``properties`` にも追加します。 |Fess| があとで作成するドキュメントインデックス（空のクラスターで
   初めて起動したとき、および :doc:`../admin/maintenance-guide` の再インデクシング）にも定義が
   入ります。 ``doc.json`` はアップグレードで新しい版のファイルに置き換わるので、アップグレード後に
   もう一度追加してください。
...
.. note::
   ...
   フィールドの型は、そのフィールドを持つ最初の文書を登録したときに決まり、あとからマッピングの
   API で変えることはできません。すでに文字列や ``float`` 型で登録したフィールドの型を変えるには、
   上記の手順 2 のとおり ``doc.json`` に定義を追加してから、 :doc:`../admin/maintenance-guide` で
   「エイリアスの更新」を有効にして再インデクシングを実行します。 ``doc.json`` から作成した新しい
   インデックスに文書がコピーされます。
