...
.. important::
   |Fess| 15.9以降、Groovyは ``fess-script-groovy`` プラグインとして提供され、
   **プラグインは配布物に同梱されません** 。管理画面の「システム > プラグイン」ページ、
   または ``bin/fess-setup install plugin fess-script-groovy`` でインストールしてください。
   手動でインストールする場合は、対応するJARファイル（例: ``fess-script-groovy-15.9.0.jar`` ）を
