...
.. important::
   失敗したスケジュールジョブ（ジョブログにステータス ``fail`` で記録されるもの）は、 ``ERROR`` ではなく
   ``WARN`` （ ``Failed to execute job: ...`` ）で記録されます。そのため、レベルが ``ERROR`` のときは、
   失敗したスケジュールジョブは\ **通知されません**\ 。通知を受けるにはレベルを ``WARN`` にしてください。
   その場合、ほかの ``WARN`` のイベントもすべて通知されます。
...
1. メールサーバーの設定:
   ...
   この値は ``fess_env_web.properties`` に設定します。ログ通知と「全般」設定のテストメールは、
   このファイルを読む |Fess| の Web プロセスが送信します。クローラーは
   ``fess_env_crawler.properties`` を読む別のプロセスとして動き、クロール完了のメールを送信するので、
   こちらにも同じ値を設定してください。これらのファイルは、ZIP 版では ``app/WEB-INF/classes/`` に、
   RPM/DEB 版では ``/etc/fess/`` にあります。 ``bin/fess`` は ``-Dlasta.env=web`` で |Fess| を起動するため、
   ``fess_env.properties`` にだけ設定した値は使われません。変更後は |Fess| を再起動してください。
...
.. list-table::
   ...
   * - 本番環境
     - ``ERROR``
     - 重要なエラーのみ通知し、ノイズを減らす。このレベルでは失敗したスケジュールジョブは
       通知されないため、必要なら ``WARN`` にする
   * - ステージング環境
     - ``WARN``
     - 潜在的な問題も含めて通知
   * - 開発環境
...
3. **メールサーバー設定の確認**
   ...
   メール通知を使用する場合、 ``fess_env_web.properties`` でメールサーバーが正しく
   設定されているか確認します（「セットアップ」の「メール通知」を参照）。「全般」設定の「テストメールの送信」ボタンは、
   同じ設定でメールを送信します。
