...
|Fess| 15.9 では、LLM機能が ``fess-llm-*`` プラグインとして分離されました。
...
|Fess| 15.9 では設定が「FessConfig 系」と「SystemProperty 系」の2系統に分かれています。
...
.. note::
   ...
   ``rag.llm.type`` というプロパティ名は |Fess| 15.5 までの旧名です。15.9 以降は ``rag.llm.name`` に変更されており、
...
|Fess| 15.9 では、生成パラメーター（最大トークン数、temperature等）はプロバイダーごと、
...
|Fess| 15.9 では、システムプロンプトはプロパティファイルではなく、各 ``fess-llm-*``
...
.. note::
   ...
   15.9 では ``/api/v2/`` 配下のJSONベースAPIを使用してください。
