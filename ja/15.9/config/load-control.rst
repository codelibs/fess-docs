...
.. note::
   ...
   連続失敗が3回目までは WARNING レベルでログに記録され、4回目以降は DEBUG レベルに変更されます
   （継続的な失敗によるログの肥大化を防ぐため）。監視が一度でも成功すると、失敗カウントはリセットされます。
...
- HTTP 429 ステータスコードを返却
- ``Retry-After: 60`` HTTP レスポンスヘッダーを付与
- JSON レスポンスを返却:
...
.. note::
   リクエストが拒否されると、サーバーログに INFO レベルで
   ``Rejecting request due to high CPU load: path=..., cpu=...%, threshold=...%``
   が出力されます。どのパスがどの閾値で拒否されたかを確認できます。
