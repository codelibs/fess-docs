...
- **ファイルサイズ**: 100MB を超えたらローテーション
- **保持世代数**: 同じ日付のファイルは最大10ファイル
- **保持期間**: 圧縮済みのファイル（ ``*.log.gz`` ）は、最終更新から指定の日数を過ぎるとローテーション時に削除

保持期間の既定値は以下のとおりです。 ``log4j2.xml`` の ``<Properties>`` にある値で変更できます。
