...
.. list-table::
   ...
   * - JavaScript
     - ``javascript``
     - 標準で登録されているスクリプト言語。プラグインを追加しなくても利用可能
   * - Groovy
     - ``groovy``
     - ``fess-script-groovy`` プラグインで提供。Java互換で強力な機能を提供

.. note::
   |Fess| に標準で登録されているスクリプトエンジンはJavaScriptです。
   デフォルトのスクリプト言語は ``javascript`` です（ ``Constants.DEFAULT_SCRIPT`` ）。
   Groovyは標準では組み込まれていません。識別子 ``groovy`` を使用するには、管理画面
   「システム」→「プラグイン」から ``fess-script-groovy`` プラグインをインストールしてください。
   本ページのスクリプト例はすべてGroovy構文で記述しているため、実行するには同プラグインが必要です。
   あるいは、JavaScriptエンジン向けに書き換えてください。
