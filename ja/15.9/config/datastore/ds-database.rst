...
.. list-table::
   ...
   * - ``last_crawl_time``
     - いいえ
     - 差分クロールの基準時刻。クロール完了時に自動で書き戻されます（「差分クロール」を参照）
   * - ``last_crawl_time_format``
     - いいえ
     - ``last_crawl_time`` の書式。デフォルト: ``yyyy-MM-dd HH:mm:ss``
...
``sql`` に ``${last_crawl_time}`` と書くと、前回クロールを開始した時刻に置き換えられます:

::

    sql=SELECT id, title, content, url, updated_at FROM articles WHERE updated_at > '${last_crawl_time}'

初回は ``1970-01-01 00:00:00`` に置き換えられるため、全件が対象になります。
結果セットを最後まで読み終えると、今回のクロール開始時刻が ``last_crawl_time``
としてデータストア設定に書き戻され、次回のクロールで使われます。

時刻の書式は ``last_crawl_time_format`` で変更できます（デフォルト
``yyyy-MM-dd HH:mm:ss`` ）。データベースがタイムスタンプリテラルとして受け付ける
書式を指定してください。

基準時刻はクロールの **開始時刻** です。クロール中に更新された行は次回のクロールで
取得されます。また、クロールが途中で停止した場合は書き戻しを行いません。

.. warning::
   差分クロールでは、削除された行を検知できません。

   差分クロールを有効にすると、|Fess| が「前回のクロールに含まれなかった
   ドキュメント」をインデックスから削除する処理（ ``delete_old_docs`` ）が
   自動的に無効化されます。これを無効化しないと、変更のなかったドキュメントが
   毎回すべて削除されてしまうためです。

   その結果、データベースから削除された行に対応するドキュメントは、有効期限が
   切れるまでインデックスに残ります。定期的に全件クロール（ ``${last_crawl_time}``
   を含まない設定）を実行してください。

   ``delete_old_docs`` をデータストア設定に明示した場合は、そちらが優先されます。

IDによる範囲指定など、従来どおり ``sql`` に条件を直接書く方法も引き続き使えます:
