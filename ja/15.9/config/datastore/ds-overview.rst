...
.. list-table::
   ...
   * - ``script_type``
     - ``javascript``
     - インデックスフィールドのマッピングに使用するスクリプトエンジンの種類。JavaScriptが |Fess| に
       組み込まれているスクリプトエンジンであり、データストア設定の新規作成画面では
       ``script_type=javascript`` があらかじめ入力されます。 ``groovy`` は
       ``fess-script-groovy`` プラグインにより引き続き利用できます。 ``script_type`` が記録されて
       いない設定はGroovyとして扱われるため、15.9より前に作成された設定はそのまま動作し続けます。
       詳細は :doc:`../scripting-overview` を参照してください。
