...
データストアクロールは、リレーショナルデータベース（MySQL・PostgreSQL・Oracle など）や CSV ファイルをはじめ、Web サイトやファイル以外のさまざまなデータソースを |Fess| のインデックスに登録し、全文検索できるようにする機能です。対応するデータソースはデータストアコネクタプラグインによって拡張できます。
...
``script_type`` パラメーターで、次項のスクリプトを評価するスクリプトエンジンを指定します。
新規作成時は ``script_type=javascript`` があらかじめ入力されています。
``script_type`` を指定しない場合、スクリプトはGroovyとして評価されます。

スクリプト
::::::::

データストアから取得した値をどのフィールドに設定するかなどを指定します。
式は、上記の ``script_type`` パラメーターで指定したスクリプトエンジンの構文で記述します
（既定はJavaScriptです）。

.. warning::

   JavaScriptでは、各行を末尾のセミコロンを付けない式として記述してください。文のブロックと
   してしか解析できない行（末尾にセミコロンがあるもの、 ``let`` / ``const`` による宣言など）
   は、明示的な ``return`` を含まない限り ``null`` と評価され、 ``null`` と評価された
   マッピングはそのフィールドを設定しません。エラーもログも出力されず、フィールドが黙って
   空になること以外に症状はありません。
   :ref:`javascript-statement-null` （ :doc:`../config/scripting-javascript` ）を参照して
   ください。
...
値の側は、 ``script_type`` パラメーターで指定したスクリプトエンジンの構文で記述します。
...
.. |image0| image:: ../../../resources/images/ja/15.9/admin/dataconfig-1.png
.. |image1| image:: ../../../resources/images/ja/15.9/admin/dataconfig-2.png
