...
.. note::
   ...
   **インデックスを close / open するまで反映されません**\ 。
   ...
   対象を再クロールする必要があります。
   反映の手順は :ref:`dict-apply-changes` を参照してください。
...
辞書を保存しただけでは、時間をおいても検索結果は変わりません。保存した内容は OpenSearch の
configsync プラグインがおよそ 1 分ごとに辞書ファイルへ書き出しますが、アナライザーが辞書を
読み込むのはインデックスを開いたときだけなので、開いているインデックスは古い辞書のまま動きます。
辞書を編集したら、ドキュメントインデックスをリロードしてください。

1. 左メニューの [システム情報 > メンテナンス] を開きます。
2. 「ドキュメントインデックスのリロード」の [リロード] ボタンをクリックします。

このボタンは、保存済みの辞書をファイルへ書き出してから、 ``fess.update`` エイリアスが指す
インデックスを close して open します。定期的な書き出しを待つ必要はありません。同義語のように
検索時に効く辞書は、インデックスが開いた時点で反映されます。インデックス時に効く辞書は、
さらに対象の文書を再クロールしてください。

.. warning::

   インデックスを close している間と、open したあとシャードが割り当てられるまでの間は、
   そのインデックスを検索できません。検索がエラーになったり 0 件になったりします。インデックスが
   大きいほどこの時間は長くなるので、アクセスの少ない時間帯に実行してください。

管理画面を使わずにスクリプトなどから行う場合は、OpenSearch に同じ操作を送ります::
...
``_configsync/flush`` は保存済みの辞書をすぐにファイルへ書き出します。省略する場合は、辞書を
保存してから 1 分以上待ってからインデックスを close してください。
