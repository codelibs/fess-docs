...
.. note::
   検索語の ``sort:`` に、ソート対象フィールド一覧にないフィールド名や ``asc`` ・ ``desc`` 以外のソート順を指定しても、エラーにはなりません。
   その場合、 ``sort:`` はソート条件として扱われず、検索語全体の記号（ ``:`` など）を通常の文字として扱って検索し直します。
   ソートされないうえに、多くの場合は検索結果が0件になります。

   検索APIの ``sort`` パラメーターに同じ値を指定した場合は、エラー（400 Bad Request）になります。

.. |image0| image:: ../../../resources/images/ja/15.9/user/search-sort-1.png
.. pdf            :width: 300 px
