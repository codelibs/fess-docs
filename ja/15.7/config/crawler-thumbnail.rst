...
.. note::
   ...
   ``mimetype`` 引数は ``image`` タイプでのみ使用され、ImageMagickに渡すフォーマットヒントを決定します。
   ヒントが有効になるMIMEタイプは ``image/gif`` 、 ``image/tiff`` 、 ``image/png`` 、 ``image/jpeg`` 、
   ``image/bmp`` （およびそのエイリアス）、Photoshop（PSD）系のMIMEタイプです。
   ``pdf`` 、 ``msoffice`` 、 ``ps`` タイプでは ``mimetype`` 引数は無視されます。
...
ドキュメントIDを10文字ごとに分割し、分割ごとに ``_<0-9>`` 形式（ハッシュ値）のサブディレクトリを作成します。
そのため、ディレクトリの階層数はドキュメントIDの長さに依存します。最下層のファイル名は ``<ドキュメントID>.png`` です。
