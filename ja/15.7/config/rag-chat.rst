...
|Fess| 15.7 では、LLM機能が ``fess-llm-*`` プラグインとして分離されました。
...
|Fess| 15.7 では設定が「FessConfig 系」と「SystemProperty 系」の2系統に分かれています。
...
.. note::
   ...
   ``rag.llm.type`` というプロパティ名は |Fess| 15.5 までの旧名です。15.7 以降は ``rag.llm.name`` に変更されており、
...
|Fess| 15.7 では、生成パラメーター（最大トークン数、temperature等）はプロバイダーごと、
...
|Fess| 15.7 では、システムプロンプトはプロパティファイルではなく、各 ``fess-llm-*``
...
.. note::
   ...
   15.7 では ``/api/v2/`` 配下のJSONベースAPIを使用してください。
...
2. 対応する ``fess-llm-*`` プラグインがインストールされているか
   ...
   - Docker の場合: ``FESS_PLUGINS=fess-llm-gemini:15.7.0`` （または ``fess-llm-openai`` / ``fess-llm-ollama``）が指定されているか
   - パッケージインストールの場合: ``app/WEB-INF/plugin/`` に該当の JAR ファイルが配置されているか
   - 起動ログに ``Installing fess-llm-XXX-15.7.0.jar`` が出力されているか
...
6. LLMプロバイダーへのネットワーク接続が可能か
   ...
   - Proxy 経由が必要なら、 ``fess_config.properties`` の ``http.proxy.host`` / ``http.proxy.port`` （必要に応じて ``http.proxy.username`` / ``http.proxy.password`` ）を設定してください。Docker環境では ``FESS_JAVA_OPTS`` に ``-Dfess.config.http.proxy.host=... -Dfess.config.http.proxy.port=...`` を追加します（ |Fess| 15.7 以降、LLMクライアントは |Fess| 共通のプロキシ設定を参照します）
