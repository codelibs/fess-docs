...
このコネクタはJWT（Server Authentication）でエンタープライズに接続し、
エンタープライズに所属する各ユーザーになりすまして（impersonation）、
それぞれのユーザーがアクセスできるファイルを再帰的にクロールします。
クロール対象のユーザーは ``filter_term`` パラメーターで絞り込めます。
...
.. list-table::
   ...
   * - ``ignore_folder``
     - ``true``
     - フォルダをインデックス対象外にするかどうか。現在の実装ではフォルダ自体はインデックスされず（ファイルのみが対象）、このパラメーターは効果を持ちません。
   * - ``ignore_error``
     - ``true``
     - エラー発生時に処理を継続するかどうか
   * - ``filter_term``
     - （なし）
     - クロール対象のエンタープライズユーザーを絞り込むフィルター条件。指定しない場合はすべてのエンタープライズユーザーが対象になります。
   * - ``fields``
     - （全フィールド）
...
.. list-table::
   ...
   * - ``base_url``
     - ``https://app.box.com``
     - ブラウザでファイルを開くURL（``file.url``）を構築するためのベースURL。Box SDKが使用するAPIエンドポイントには影響しません。
   * - ``max_retry_count``
     - ``10``
     - API呼び出しの最大リトライ回数
   * - ``proxy_host``
...
``file.api`` フィールドが提供する ``BoxFileAPI`` オブジェクトを通じて、
Boxのコラボレーション情報を |Fess| の検索ロールにマッピングできます。
``file.api.collaborationRoles`` は、ファイルにアクセスできるユーザーおよび
グループに対応する検索ロールのリストを返します。
...
.. note::
   ``file.api.collaborationRoles`` はファイルごとにコラボレーション情報を
   取得するため、Box APIの呼び出し回数が増え、クロールに時間がかかる場合があります。

固定のロールをすべてのファイルに割り当てる場合は、次のように指定します:
