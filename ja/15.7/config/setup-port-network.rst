...
この設定はクローラーのHTTP通信にのみ適用されます。SSO認証や外部API連携などのHTTP通信には適用されません。
...
.. note::
   Fessアプリケーション全体（SSO、LLM連携等を含む）でプロキシを使用する場合は、環境変数 ``FESS_PROXY_HOST``、``FESS_PROXY_PORT`` を設定してください。詳細は後述の「環境変数によるプロキシ設定」を参照してください。
...
.. note::
   ``fess_config.properties`` の ``http.proxy.*`` 設定はクローラーのデフォルトプロキシとして使用されます。
   SSO認証やLLM連携などを含むFessアプリケーション全体でプロキシを使用する場合は、上記の環境変数を設定してください。
