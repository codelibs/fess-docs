...
``FESS_HEAP_SIZE`` は ``-Xms`` と ``-Xmx`` を同じ値に設定しますが、初期ヒープサイズと最大ヒープサイズを個別に制御したい場合は、``FESS_MIN_MEM`` と ``FESS_MAX_MEM`` 環境変数を使用します。
...
.. list-table:: プラットフォーム別デフォルトメモリ設定
   :header-rows: 1
   :widths: 30 35 35
...
.. note::
   ``FESS_HEAP_SIZE`` を設定した場合、``FESS_MIN_MEM`` と ``FESS_MAX_MEM`` は無視されます。
...
.. list-table::
   ...
   * - ``-XX:MaxGCPauseMillis=60000``
     - GC一時停止の最大目標時間(60秒)。クローラーではGCを積極的に制限しない設定(通常のアプリケーションでは200ms程度が一般的)
...
OpenSearch のヒープメモリは、``config/jvm.options`` で設定することを推奨します。

``config/jvm.options`` を編集:
...
.. note::
   ``OPENSEARCH_HEAP_SIZE`` 環境変数は OpenSearch 3.x 以降では非推奨です。代わりに ``config/jvm.options`` または ``OPENSEARCH_JAVA_OPTS`` 環境変数を使用してください。
