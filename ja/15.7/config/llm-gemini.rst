...
|Fess| 15.7では、Gemini連携機能は ``fess-llm-gemini`` プラグインとして提供されています。
...
|Fess| 15.7では、LLMプロバイダーの選択（ ``rag.llm.name`` ）は管理画面または ``system.properties`` で行い、AI検索モード機能の有効化やGemini固有の設定は ``fess_config.properties`` で行います。
...
|Fess| 15.7.1 以降、APIキーは ``x-goog-api-key`` HTTPリクエストヘッダで送信されます（Googleの推奨方式）。
...
|Fess| 15.7.1 以降、Geminiクライアントは |Fess| 全体のHTTPプロキシ設定を共有します。 ``fess_config.properties`` で以下のプロパティを指定してください。
