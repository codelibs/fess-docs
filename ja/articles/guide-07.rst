...
- ``fess-ds-google``: Google Drive / Google Suite のクロール
- ``fess-ds-office365``: SharePoint Online / OneDrive のクロール
...
3. パラメータの設定
   ...
   - サービスアカウントの認証情報
   - 対象のドライブ ID またはフォルダ ID
   - クロール対象のファイル形式（MIME タイプ）
...
1. ［クローラ］ > ［データストア］ > ［新規作成］
2. ハンドラ名: Office365 を選択
3. パラメータの設定

   - テナント ID、クライアント ID、クライアントシークレット
   - 対象のサイト URL
   - ドキュメントライブラリの指定
...
管理画面の ［全般設定］ > ［サムネイル画像］ で、サムネイル生成を有効にします。
