...
これにより、ブラウザーで表示するHTMLを出力するサーバーと Fess サーバーが同じドメインに存在する場合はJSONを利用することができますが、異なる場合はCORS(Cross-Origin Resource Sharing)を利用する必要があります。
...
5ではdoSearchがイベント登録されたときに渡されたパラメーターnaviの値を判定して、表示開始位置を修正します。
