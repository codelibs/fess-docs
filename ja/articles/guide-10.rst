...
Fess のスケジューラには、クロール完了時に通知を送る機能があります。
Slack やメールで通知を受け取ることで、クロールの完了や異常終了を即座に把握できます。

スケジューラの設定で、ジョブ完了時のスクリプトとして通知処理を記述できます。
...
Fess の管理画面の ［システム］ > ［バックアップ/リストア］ から、設定データのバックアップを取得できます。
...
- 失敗 URL の確認: ［クロール情報］ > ［失敗URL一覧］ で詳細を確認
...
クロール時にアクセスできなかった URL は「失敗 URL」として記録されます。
管理画面の ［クロール情報］ > ［失敗URL一覧］ で確認できます。

失敗 URL が多い場合は、以下を確認します。
...
- **クローラログ**: 管理画面の ［ログ］ > ［クロールログ］
- **ジョブログ**: 管理画面の ［ログ］ > ［ジョブログ］
- **検索ログ**: 管理画面の ［ログ］ > ［検索ログ］
