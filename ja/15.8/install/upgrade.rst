...
またはそれに失敗した場合、ユーザーはグループ・ロールに紐づく権限だけを欠いた状態になります
（ユーザー自身のユーザーレベルの権限と、\ ``entraid.default.groups``\ ・\ ``entraid.default.roles``\ に
設定したグループ・ロールは、最初のリクエストから保持されます）。解決中は検索画面にその旨のメッセージが
表示され、失敗した場合は別のメッセージが表示されます。アクセストークンが更新されるたびに解決が
...
バックグラウンドで解決することの副作用として、Entra IDでログインした直後の約1秒間は、解決済みの
ロールがまだ分かりません。そのため、管理者は管理ダッシュボードではなく検索画面にリダイレクトされ、
その間に管理画面を開いても検索画面に戻されます。この間にアクセスが許可されてしまうことはなく、
拒否されるだけです。解決の完了後に開き直せば、ログインし直さなくても正常にアクセスできます。
この時間帯をなくしたい場合は、\ ``entraid.default.roles``\ に管理者ロールを設定してください。
