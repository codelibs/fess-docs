...
.. note::
   ...
   |Fess| 15.8 では、API が **v2** に刷新されました。従来の ``/api/v1`` の
...
.. list-table:: error の要素
   ...
   * - details
     - 追加の構造化情報を含む任意のオブジェクト（省略される場合があります）。一部のエンドポイントのみが付与します。例えば :doc:`api-health` では、検索エンジンクラスターのスナップショットが ``error.details.engine`` に埋め込まれます。
