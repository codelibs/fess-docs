...
デフォルトではキーワード（BM25）検索が使用されます。この標準パイプラインを再利用しているため、Semantic Searchプラグインを
導入・設定すると、そのsemantic（ベクトル）サーチャーはAI検索モードの検索ステップを含むすべての検索でRank Fusionに参加します。
AI検索モード専用の設定は不要です。詳細は :doc:`rank-fusion` を参照してください。
