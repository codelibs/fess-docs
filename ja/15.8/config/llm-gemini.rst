===================================
Google Geminiの設定（AI検索 / RAG）
===================================

概要
====

このページでは、|Fess| がGoogle Geminiを **AI検索モード（RAG: Retrieval-Augmented Generation）** で利用できるように ``fess-llm-gemini`` プラグインを設定する方法を説明します。AI検索モードは、自然言語の質問に対して社内の検索インデックスから引用元付きで回答します。|Fess| はGoogle AI API（Generative Language API）を呼び出し、Geminiモデルでクロール済みドキュメントに対するRAGを実行します。
...
Gemini連携機能は ``fess-llm-gemini`` プラグインとして提供されています。
Geminiを使用するには、プラグインのインストールが必要です。

1. `fess-llm-gemini-15.8.0.jar` をダウンロードします
...
LLMプロバイダーの選択（ ``rag.llm.name`` ）は管理画面または ``system.properties`` で行い、AI検索モード機能の有効化やGemini固有の設定は ``fess_config.properties`` で行います。
...
APIキーは ``x-goog-api-key`` HTTPリクエストヘッダで送信されます（Googleの推奨方式）。
...
- ``FESS_PLUGINS=fess-llm-gemini:15.8.0`` で ``run.sh`` がプラグイン JAR を自動取得して ``app/WEB-INF/plugin/`` に配置します
...
Geminiクライアントは |Fess| 全体のHTTPプロキシ設定を共有します。 ``fess_config.properties`` で以下のプロパティを指定してください。
