...
このページでは、|Fess| がOpenAIを **AI検索モード（RAG: Retrieval-Augmented Generation）** で利用できるように ``fess-llm-openai`` プラグインを設定する方法を説明します。AI検索モードは、自然言語の質問に対して社内の検索インデックスから引用元付きで回答します。|Fess| はOpenAI APIを呼び出し、GPTモデルでクロール済みドキュメントに対するRAGを実行します。
...
1. `fess-llm-openai-15.8.0.jar` をダウンロードします
...
- ``FESS_PLUGINS=fess-llm-openai:15.8.0`` で ``run.sh`` がプラグイン JAR を自動取得して ``app/WEB-INF/plugin/`` に配置します
