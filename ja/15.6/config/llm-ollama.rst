...
- ``FESS_PLUGINS=fess-llm-ollama:15.6.0`` で ``run.sh`` がプラグイン JAR を自動取得して ``app/WEB-INF/plugin/`` に配置します
...
|Fess| 15.6.1 以降、Ollamaクライアントは |Fess| 全体のHTTPプロキシ設定を共有します。Ollamaサーバーへの接続にプロキシを経由する必要がある場合（リモートOllamaサーバー利用時など）、 ``fess_config.properties`` で以下のプロパティを指定してください。
