...
|Fess| 15.6 では設定が「FessConfig 系」と「SystemProperty 系」の2系統に分かれています。
...
.. important::
   ...
   ``RAG_CHAT_ENABLED=true`` や ``RAG_LLM_GEMINI_API_KEY=...`` のような単純な環境変数は |Fess| では自動マッピングされません。
   すべて ``FESS_JAVA_OPTS`` の中で ``-Dfess.config.*`` または ``-Dfess.system.*`` として指定する必要があります。

.. note::

   ``rag.llm.type`` というプロパティ名は |Fess| 15.5 までの旧名です。15.6 以降は ``rag.llm.name`` に変更されており、
...
エンドポイント: ``GET|POST /api/v1/chat/stream``
...
2. 対応する ``fess-llm-*`` プラグインがインストールされているか
   ...
   - Docker の場合: ``FESS_PLUGINS=fess-llm-gemini:15.6.0`` （または ``fess-llm-openai`` / ``fess-llm-ollama``）が指定されているか
   - パッケージインストールの場合: ``app/WEB-INF/plugin/`` に該当の JAR ファイルが配置されているか
   - 起動ログに ``Installing fess-llm-XXX-15.6.0.jar`` が出力されているか
