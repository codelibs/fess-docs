...
|Fess| 15.6 から、AI検索モード（RAG Chat）機能は ``fess-llm-*`` プラグインとして分離されました。
...
各オーバーレイは ``FESS_PLUGINS`` で対応プラグインを自動取得し、 ``FESS_JAVA_OPTS`` に
``-Dfess.config.rag.chat.enabled=true`` と ``-Dfess.system.rag.llm.name={provider}`` を設定するように作られています。
...
.. tip::
   ...
   これらの値は OpenSearch に永続化されるため、次回以降の起動時には JVM オプションよりも優先されます。
